import type { EvidenceMediaType } from "./fulfillmentApi";

/**
 * Client-side hint only; the backend remains the authoritative validator of
 * accepted evidence media types and evidence types.
 */
export const ACCEPTED_EVIDENCE_MEDIA_TYPES: readonly EvidenceMediaType[] = [
  "application/pdf",
...
export const ACCEPTED_EVIDENCE_FILE_INPUT_ACCEPT =
  ".pdf,.docx,.jpg,.jpeg,.png,.mp4,application/pdf,application/vnd.openxmlformats-officedocument.wordprocessingml.document,image/jpeg,image/png,video/mp4";

export function inferEvidenceMediaType(
  file: File,
...
export class DirectUploadError extends Error {
  constructor(readonly status: number) {
    super("Evidence bytes could not be uploaded to storage.");
 ...
 */
export function putEvidenceBytes(
  uploadUrl: string,
