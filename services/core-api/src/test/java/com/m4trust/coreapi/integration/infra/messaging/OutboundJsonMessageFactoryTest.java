...
class OutboundJsonMessageFactoryTest {
  ...
  void createsUtf8JsonMessage() {
    ...
    assertEquals(
        MessageProperties.CONTENT_TYPE_JSON, message.getMessageProperties().getContentType());
    assertEquals(
        StandardCharsets.UTF_8.name(), message.getMessageProperties().getContentEncoding());
  }
}
