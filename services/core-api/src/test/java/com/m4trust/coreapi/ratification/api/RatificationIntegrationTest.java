 ...
 * plus the §8 minimum invariants that need a real Deal + document + accepted rule-set: the
 * withdrawal-vs-last-approval race in both orderings, and the approve-vs-supersede race. Each
 * "race" here is proven by ordering two real, fully-committed HTTP calls rather than true thread
 * concurrency: DealCancelRaceIntegrationTest already proves the shared Deal-row lock (used by every
 * one of these mutations) actually blocks a second transaction until the first commits, so
 * whichever call completes first is exactly what a real concurrent race would serialize to.
 */
@SpringBootTest
...
class RatificationIntegrationTest {
  ...
  void happyPathCreateApproveApproveActivatesTheDealAndPackage() throws Exception {
    ...
    MvcResult created =
        createPackage(dealId, buyer, 1, 5_000, "TRY")
    ...
    mockMvc
        .perform(
            get(packagePath(dealId, packageId))
    ...
    approve(dealId, packageId, buyer, 0)
        ...
        .andExpect(
            jsonPath("$.approvals[?(@.legalEntityId=='" + buyer.legalEntityId + "')].status")
                .value("APPROVED"));
    ...
    mockMvc
        .perform(
            patch("/api/v1/deals/" + dealId)
                ...
                .content(
                    "{\"title\": \"Post-activation edit\", \"description\": null, \"expectedVersion\": 3}"))
  ...
  void invalidCommercialTermsAndMissingIdempotencyKeyAreRejected() throws Exception {
    ...
    assertEquals(
        0, jdbc.queryForObject("SELECT count(*) FROM ratification_package", Integer.class));
  }
  ...
  private org.springframework.test.web.servlet.ResultActions approve(
    ...
    return mockMvc.perform(
        post(packagePath(dealId, packageId) + "/approve")
            .with(user(actor.userId.toString()))
            .with(csrf())
            .header(LEGAL_ENTITY_HEADER, actor.legalEntityId)
            .header("Idempotency-Key", UUID.randomUUID())
  ...
  private org.springframework.test.web.servlet.ResultActions reject(
      UUID dealId, UUID packageId, Principal actor, long expectedPackageVersion) throws Exception {
    return mockMvc.perform(
        post(packagePath(dealId, packageId) + "/reject")
            .with(user(actor.userId.toString()))
            .with(csrf())
            .header(LEGAL_ENTITY_HEADER, actor.legalEntityId)
            .header("Idempotency-Key", UUID.randomUUID())
  ...
  /**
   * Buyer-initiated DRAFT Deal with parties, an AVAILABLE document, and an ACCEPTED rule-set:
   * READY.
   */
