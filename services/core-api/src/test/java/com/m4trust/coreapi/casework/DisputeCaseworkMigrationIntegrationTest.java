...
class DisputeCaseworkMigrationIntegrationTest {
  ...
  void enforcesDisputeCaseSnapshotAndCommentInvariants() {
    ...
    assertThrows(
        ...
        () ->
            jdbc.update(
                "UPDATE dispute_case SET opening_legal_entity_id=? WHERE id=?",
                fixture.sellerId(),
                firstCase),
    ...
    assertThrows(
        ...
        () ->
            jdbc.update(
                ...
                open.packageId(),
                open.actorTenantId(),
                open.openingLegalEntityId(),
                open.openingUserId()),
        "RESOLVED insert is rejected in Slice 14A");

    UUID rejectedEvidence =
        rejectedEvidence(open.dealId(), open.milestoneId(), open.fulfillmentId());
    assertDoesNotThrow(
        () ->
            jdbc.update(
                """
    ...
    assertThrows(
        ...
        () ->
            jdbc.update(
                "UPDATE dispute_evidence_snapshot SET file_name='changed' WHERE id=?", snapshot),
  ...
  void allowsCrossTenantOpeningWhenActorTenantMatchesEntityAndUser() {
    ...
    jdbc.update(
        "INSERT INTO tenant_user (user_id, tenant_id) VALUES (?, ?)", crossUser, actorTenant);
    jdbc.update(
        """
  ...
  private static UUID disputeCase(
      OpenContext open, String status, Instant acknowledgedAt, Instant withdrawnAt) {
  ...
  private static UUID evidenceSnapshot(
      UUID disputeCaseId, UUID dealId, UUID evidenceId, UUID videoJobId, UUID videoResultId) {
    UUID id = UUID.randomUUID();
    jdbc.update(
        """
                INSERT INTO dispute_evidence_snapshot (
  ...
  private static OpenContext openContext(Fixture fixture) {
    ...
    UUID packageId =
        ratificationPackage(fixture.dealId(), fixture.buyerId(), fixture.sellerId(), snapshotId);
    jdbc.update(
        "UPDATE deal SET deal_status='ACTIVE', buyer_legal_entity_id=?, seller_legal_entity_id=?, current_ratification_package_id=? WHERE id=?",
        fixture.buyerId(),
        fixture.sellerId(),
        packageId,
        fixture.dealId());
    ...
    UUID evidenceId =
        evidence(UUID.randomUUID(), fixture.dealId(), milestoneId, fulfillmentId, sha256);
    ...
    UUID otherDealEvidenceId =
        evidence(
            UUID.randomUUID(), fixture.otherDealId(), otherMilestoneId, otherFulfillmentId, sha256);
  ...
  private static UUID ratificationPackage(
      UUID dealId, UUID buyerId, UUID sellerId, UUID snapshotId) {
  ...
  private static UUID evidence(
      UUID id, UUID dealId, UUID milestoneId, UUID fulfillmentId, String sha256) {
    jdbc.update(
        """
                INSERT INTO fulfillment_evidence_submission (
                    id, deal_id, milestone_id, fulfillment_id, evidence_type, media_type, file_name,
