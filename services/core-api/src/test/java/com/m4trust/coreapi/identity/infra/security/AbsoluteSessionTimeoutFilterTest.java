...
class AbsoluteSessionTimeoutFilterTest {
  ...
  @Test
  void invalidatesAnActiveSessionAtTheServerSideAbsoluteDeadline() throws Exception {
    MockHttpSession session = new MockHttpSession();
    Instant deadline = Instant.ofEpochMilli(session.getCreationTime()).plus(Duration.ofHours(8));
