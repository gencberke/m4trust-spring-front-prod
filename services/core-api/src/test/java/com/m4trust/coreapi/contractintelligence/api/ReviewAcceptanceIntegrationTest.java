...
class ReviewAcceptanceIntegrationTest {
  ...
  void setUp() {
    jdbc.execute(
        "TRUNCATE TABLE integration_outbox_event, integration_inbox_event, dispute_comment, dispute_evidence_snapshot, dispute_case, fulfillment_video_analysis_result, fulfillment_video_analysis_job, fulfillment_evidence_submission, fulfillment_milestone_rule_reference, fulfillment_milestone, fulfillment, release_dispatch, release_operation, settlement, payment_dispatch, payment_operation, funding_unit, funding_plan, contract_intelligence_rule_set_version, contract_intelligence_extraction_result_version, contract_intelligence_analysis_job, http_idempotency_record, audit_record, deal_invitation, deal_participant, document, ratification_package_approval, ratification_package, ratification_package_snapshot, deal, legal_entity_membership, legal_entity, tenant_user, tenant, identity_user");
    user = UUID.randomUUID();
    tenant = UUID.randomUUID();
    entity = UUID.randomUUID();
    deal = UUID.randomUUID();
    document = UUID.randomUUID();
    seedActor(user, entity, "initiator");
    seedDeal();
    seedAnalysis(rules());
    failAudit.set(false);
  }
  ...
  @Test
  void acceptsAllDecisionKindsPersistsExactRuleSetAndProjectsDeal() throws Exception {
    var result =
        accept(
                body(
                    decisions(
          ...
          """)),
            ...
            .andExpect(jsonPath("$.excludedRuleReferences[0]").value("exclude"))
            .andReturn();
    String version =
        jdbc.queryForObject(
            "select id::text from contract_intelligence_rule_set_version where deal_id=?",
            String.class,
            deal);
    assertEquals(
        "ACCEPTED",
    ...
    mvc.perform(get("/api/v1/deals/" + deal).with(user(user.toString())).header(h(), entity))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.lifecycle").value("RATIFICATION"))
        .andExpect(jsonPath("$.currentRuleSet.version").value(1))
        .andExpect(jsonPath("$.availableActions.canReviewExtraction").value(false));
    UUID participant = UUID.randomUUID(), participantEntity = UUID.randomUUID();
    seedActor(participant, participantEntity, "participant");
    participant(participantEntity);
    mvc.perform(
            get("/api/v1/deals/" + deal + "/rule-set-versions")
                .with(user(participant.toString()))
                .header(h(), participantEntity))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.items[0].id").value(version));
    mvc.perform(
            get("/api/v1/deals/" + deal + "/rule-set-versions/" + version)
                .with(user(participant.toString()))
                .header(h(), participantEntity))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.rules[1].structuredValue.amountMinor").value(1234));
    mvc.perform(
            get("/api/v1/deals/" + UUID.randomUUID() + "/rule-set-versions/" + version)
                .with(user(participant.toString()))
                .header(h(), participantEntity))
        .andExpect(status().isNotFound())
        .andExpect(jsonPath("$.code").value("RULE_SET_VERSION_NOT_FOUND"));
  }
  ...
  void participantReadsButCannotAcceptAndOutsiderGetsContractSpecificNotFound() throws Exception {
    ...
    seedActor(p, pe, "p");
    participant(pe);
    seedActor(outsider, oe, "o");
    mvc.perform(
            get("/api/v1/deals/" + deal + "/extraction-review")
                .with(user(p.toString()))
                .header(h(), pe))
        .andExpect(status().isOk());
    acceptAs(
            p,
        ...
        .andExpect(jsonPath("$.code").value("DEAL_REVIEW_ACCEPTANCE_FORBIDDEN"));
    mvc.perform(
            get("/api/v1/deals/" + deal + "/rule-set-versions/" + UUID.randomUUID())
                .with(user(outsider.toString()))
                .header(h(), oe))
        .andExpect(status().isNotFound())
        .andExpect(jsonPath("$.code").value("RULE_SET_VERSION_NOT_FOUND"));
    mvc.perform(
  ...
  @Test
  void concurrentAcceptsCreateExactlyOneVersionWithoutDeadlock() throws Exception {
    ...
    try {
      Future<Integer> first =
          executor.submit(
              () -> {
                start.await();
                return accept(completeBody(), UUID.randomUUID())
                    .andReturn()
                    .getResponse()
                    .getStatus();
              });
  ...
  private org.springframework.test.web.servlet.ResultActions accept(String b, UUID key)
      throws Exception {
    return acceptAs(user, entity, b, key);
  }

  private org.springframework.test.web.servlet.ResultActions acceptAs(
      UUID u, UUID e, String b, UUID key) throws Exception {
    return mvc.perform(
        post("/api/v1/deals/" + deal + "/extraction-review/accept")
            .with(user(u.toString()))
            .with(csrf())
            .header(h(), e)
  ...
  private String body(String decisions) {
    return "{\"analysisId\":\""
        + analysis
        + "\",\"expectedVersion\":0,\"decisions\":"
        + decisions
        + "}";
  }

  private String body(UUID analysisId) {
    return bodyWithVersion(0, analysisId);
  }

  private String bodyWithVersion(long expectedVersion, UUID analysisId) {
    return "{\"analysisId\":\""
        + analysisId
        + "\",\"expectedVersion\":"
        + expectedVersion
  ...
  private String completeBody() {
    return bodyWithVersion(0, analysis);
  }

  private String completeDecisions() {
    return decisions(
        "{\"decision\":\"KEPT\",\"ruleReference\":\"keep\"},{\"decision\":\"KEPT\",\"ruleReference\":\"modify\"},{\"decision\":\"KEPT\",\"ruleReference\":\"exclude\"}");
  }

  private String decisions(String values) {
    return "[" + values + "]";
  }
  ...
  private int count(String t) {
    return jdbc.queryForObject("select count(*) from " + t, Integer.class);
  }

  private String text(String q, Object a) {
    return jdbc.queryForObject(q, String.class, a);
  }

  private void seedActor(UUID u, UUID e, String label) {
    jdbc.update(
        "insert into identity_user(id,email,password_hash,display_name,enabled) values(?,?, 'x',?,true)",
        u,
        u + "@test",
        label);
    jdbc.update("insert into tenant(id) values(?) on conflict do nothing", tenant);
    jdbc.update("insert into tenant_user(user_id,tenant_id) values(?,?)", u, tenant);
    jdbc.update(
        ...
        label + e);
    jdbc.update(
        "insert into legal_entity_membership(id,tenant_id,legal_entity_id,user_id,role) values(?,?,?,?, 'ADMIN')",
        UUID.randomUUID(),
        tenant,
        e,
        u);
  ...
  private void seedDeal() {
    jdbc.update(
        "insert into deal(id,tenant_id,reference,title,deal_status,initiator_legal_entity_id,created_by) values(?,?, 'DL-0000000001','d','DRAFT',?,?)",
        deal,
        tenant,
        entity,
        user);
  ...
  private void participant(UUID e) {
    jdbc.update(
        "insert into deal_participant(deal_id,tenant_id,legal_entity_id,legal_entity_tenant_id) values(?,?,?,?)",
        deal,
        tenant,
        e,
        tenant);
  ...
  private void seedAnalysis(String canonical) {
    analysis = UUID.randomUUID();
    extraction = UUID.randomUUID();
    jdbc.update(
        "insert into contract_intelligence_analysis_job(id,tenant_id,deal_id,document_id,object_version,input_sha256,status,requested_at,processing_started_at,completed_at,version) values(?,?,?,?,?,'"
            + SHA
            + "','REVIEW_REQUIRED',current_timestamp,current_timestamp,current_timestamp,0)",
        analysis,
        tenant,
        deal,
        document,
        "v");
    jdbc.update(
        "insert into contract_intelligence_extraction_result_version(id,analysis_job_id,schema_version,canonical_result,created_at) values(?,?,'1.0.0',cast(? as jsonb),current_timestamp)",
        extraction,
        analysis,
        canonical);
  }
  ...
  private void insertRuleSetWithCrossDealPredecessor(UUID otherDeal, UUID firstDealVersion) {
    ...
    jdbc.update(
        ...
        rules());
    jdbc.update(
        "insert into contract_intelligence_rule_set_version(id,deal_id,version,source_analysis_id,source_extraction_result_version_id,created_by_user_id,created_at,previous_rule_set_version_id,previous_rule_set_deal_id,rules,excluded_rule_references) values(?,?,1,?,?,?,current_timestamp,?,?,cast('[]' as jsonb),cast('[]' as jsonb))",
        UUID.randomUUID(),
        otherDeal,
        otherAnalysis,
        otherExtraction,
  ...
  static class Fakes {
    ...
    @Bean
    @Primary
    AuditAppendPort auditAppendPort(JdbcTemplate j, AtomicBoolean f) {
      return r -> {
        if (f.get()) throw new IllegalStateException("forced");
        j.update(
            "insert into audit_record(id,tenant_id,actor_user_id,legal_entity_id,subject_type,subject_id,action,correlation_id,causation_id,occurred_at) values(?,?,?,?,?,?,?,?,?,?)",
            r.id(),
            r.tenantId(),
            r.actorUserId(),
            r.legalEntityId(),
