...
class ReviewAcceptanceIntegrationTest {
    ...
    @Test void auditFailureRollsBackEveryAcceptanceWrite() throws Exception {
        failAudit.set(true); accept(body(decisions("{\"decision\":\"KEPT\",\"ruleReference\":\"keep\"},{\"decision\":\"KEPT\",\"ruleReference\":\"modify\"},{\"decision\":\"KEPT\",\"ruleReference\":\"exclude\"}")),UUID.randomUUID()).andExpect(status().is5xxServerError());
        assertEquals(0,count("contract_intelligence_rule_set_version")); assertEquals("REVIEW_REQUIRED",text("select status from contract_intelligence_analysis_job where id=?",analysis)); assertEquals(0,count("audit_record")); assertEquals(0,count("http_idempotency_record")); assertNull(jdbc.queryForObject("select current_rule_set_version_id from deal where id=?",UUID.class,deal));
    }
