...
class VideoAnalysisCrossTenantIntegrationTest {
  ...
  @Test
  void crossTenantSellerFulfillmentVideoAnalysisRequestUsesDealHostingTenantForJob()
      throws Exception {
    mockMvc
        .perform(
    ...
    UUID fulfillmentId =
        jdbc.queryForObject("SELECT id FROM fulfillment WHERE deal_id = ?", UUID.class, dealId);
    assertEquals(
        sellerTenantId,
        jdbc.queryForObject(
    ...
    assertEquals(
        hostTenantId,
        jdbc.queryForObject("SELECT tenant_id FROM deal WHERE id = ?", UUID.class, dealId));
    assertEquals(
        hostTenantId.toString(),
        jdbc.queryForObject(
    ...
    assertEquals(
        hostTenantId,
        jdbc.queryForObject(
            "SELECT tenant_id FROM audit_record WHERE action = 'VIDEO_ANALYSIS_REQUESTED' LIMIT 1",
            UUID.class));
  ...
  private int countAudit(String action) {
    return jdbc.queryForObject(
        "SELECT count(*) FROM audit_record WHERE action = ?", Integer.class, action);
  }
