...
class FulfillmentMigrationIntegrationTest {
  ...
  void enforcesFulfillmentAndEvidenceInvariants() {
    ...
    assertThrows(
        ...
        () ->
            jdbc.update(
                "INSERT INTO fulfillment (id, deal_id, tenant_id, source_package_id, status, evidence_policy, version, created_at, updated_at) VALUES (?, ?, ?, ?, 'INVALID', 'REQUIRED', 0, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)",
                UUID.randomUUID(),
                f.otherDealId,
                f.tenant,
                packageId),
    ...
    UUID otherFulfillmentId =
        fulfillment(f.otherDealId, f.tenant, UUID.randomUUID(), "IN_PROGRESS", 0L);
    UUID otherMilestoneId =
        milestone(otherFulfillmentId, f.otherDealId, "Other", "IN_PROGRESS", 0L);
    ...
    assertThrows(
        ...
        () ->
            jdbc.update(
                "INSERT INTO fulfillment_milestone (id, fulfillment_id, deal_id, title, status, version, created_at, updated_at) VALUES (?, ?, ?, ?, 'INVALID', 0, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)",
                UUID.randomUUID(),
                fulfillmentId,
                f.dealId,
                "Bad"),
    ...
    UUID pendingEvidence =
        evidence(
            UUID.randomUUID(), f.dealId, milestoneId, fulfillmentId, "PENDING_UPLOAD", sha256, 0L);
    assertThrows(
        DataAccessException.class,
        () ->
            evidence(
    ...
    assertThrows(
        ...
        () ->
            jdbc.update(
                "UPDATE fulfillment_evidence_submission SET status='UNKNOWN' WHERE id=?",
                pendingEvidence),
    ...
    assertThrows(
        ...
        () ->
            jdbc.update(
                "UPDATE fulfillment_evidence_submission SET verified_sha256='not-a-hash' WHERE id=?",
                pendingEvidence),
        "invalid verified sha256 is rejected");

    UUID submittedEvidence =
        evidence(UUID.randomUUID(), f.dealId, milestoneId, fulfillmentId, "SUBMITTED", sha256, 1L);
    assertThrows(
        DataAccessException.class,
        () ->
            jdbc.update(
                "UPDATE fulfillment_evidence_submission SET object_key='changed' WHERE id=?",
                submittedEvidence),
    ...
    assertThrows(
        ...
        () ->
            jdbc.update(
                "UPDATE fulfillment_evidence_submission SET object_version='changed' WHERE id=?",
                submittedEvidence),
    ...
    assertThrows(
        ...
        () ->
            evidence(
                UUID.randomUUID(), f.dealId, milestoneId, fulfillmentId, "SUBMITTED", sha256, 2L),
        "at most one current SUBMITTED evidence per milestone is allowed");
    assertThrows(
        DataAccessException.class,
        () ->
            jdbc.update(
                "UPDATE fulfillment_evidence_submission SET status='SUBMITTED' WHERE id=?",
                pendingEvidence),
  ...
  private static UUID fulfillment(
      UUID dealId, UUID tenantId, UUID sourcePackageId, String status, long version) {
  ...
  private static UUID milestone(
      UUID fulfillmentId, UUID dealId, String title, String status, long version) {
    UUID id = UUID.randomUUID();
    jdbc.update(
        """
