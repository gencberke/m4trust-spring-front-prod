...
class VideoAnalysisMigrationIntegrationTest {
  ...
  void acceptsPhotoMediaTypesInJobInputCheck() {
    ...
    UUID fulfillmentId =
        fulfillment(fixture.dealId(), fixture.tenant(), UUID.randomUUID(), "REVIEW_REQUIRED", 0L);
  ...
  void enforcesVideoAnalysisJobAndResultInvariants() {
    ...
    assertThrows(
        ...
        () ->
            jdbc.update(
                "UPDATE fulfillment_video_analysis_result SET schema_version='9.9.9' WHERE id=?",
                resultId),
    ...
    UUID otherDealFulfillmentId =
        fulfillment(
            fixture.otherDealId(), fixture.tenant(), UUID.randomUUID(), "REVIEW_REQUIRED", 0L);
    UUID otherMilestoneId =
        milestone(otherDealFulfillmentId, fixture.otherDealId(), "Other", "REVIEW_REQUIRED", 0L);
    UUID otherEvidence =
        videoEvidence(
            UUID.randomUUID(),
            fixture.otherDealId(),
            otherMilestoneId,
            otherDealFulfillmentId,
            "SUBMITTED",
            sha256,
            1L);
    assertThrows(
  ...
  void allowsDealHostingJobTenantWhenFulfillmentUsesSellerActorTenant() {
    ...
    UUID fulfillmentId =
        fulfillment(fixture.dealId(), sellerActorTenant, UUID.randomUUID(), "REVIEW_REQUIRED", 0L);
    UUID milestoneId = milestone(fulfillmentId, fixture.dealId(), "Primary", "REVIEW_REQUIRED", 0L);
  ...
  private static UUID resultAvailableJob(
      UUID jobId,
      UUID tenantId,
      UUID dealId,
      UUID fulfillmentId,
  ...
  private static UUID queuedJobWithMediaType(
      UUID jobId,
      UUID tenantId,
      UUID dealId,
      UUID fulfillmentId,
  ...
  private static UUID photoEvidence(
      UUID id,
      UUID dealId,
      UUID milestoneId,
      UUID fulfillmentId,
  ...
  private static UUID videoEvidence(
      UUID id,
      UUID dealId,
      UUID milestoneId,
      UUID fulfillmentId,
