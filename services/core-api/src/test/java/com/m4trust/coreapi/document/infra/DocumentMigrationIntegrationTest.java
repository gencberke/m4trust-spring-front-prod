...
class DocumentMigrationIntegrationTest {
  ...
  void currentDocumentPointerRequiresAnAvailableDocumentOfTheSameDeal() {
    ...
    assertDoesNotThrow(() -> setCurrentDocument(fixture.dealId(), availableDocumentId));
    assertThrows(
  ...
  private void insertDeal(
      UUID dealId, UUID tenantId, UUID legalEntityId, UUID userId, String reference) {
    jdbcTemplate.update(
        """
                INSERT INTO deal (
  ...
  private static String randomReference() {
    return String.format(
        "%010d", Math.floorMod(UUID.randomUUID().getLeastSignificantBits(), 10_000_000_000L));
  }
