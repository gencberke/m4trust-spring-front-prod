...
class ContractMetadataIntegrationTest extends PostgresIntegrationTestSupport {
  ...
  void missingTokenReturns401() throws Exception {
    mockMvc.perform(get("/internal/v1/contracts")).andExpect(status().isUnauthorized());
  }
  ...
  void wrongTokenReturns401() throws Exception {
    mockMvc
        .perform(
            get("/internal/v1/contracts").header(HttpHeaders.AUTHORIZATION, "Bearer wrong-token"))
  ...
  void activeTokenReturnsBundleProjection() throws Exception {
    MvcResult result =
        mockMvc
            .perform(
                get("/internal/v1/contracts").header(HttpHeaders.AUTHORIZATION, "Bearer " + ACTIVE))
            .andExpect(status().isOk())
            .andExpect(jsonPath("$.service").value("core"))
            .andExpect(
                jsonPath("$.releaseRevision").value("0123456789abcdef0123456789abcdef01234567"))
            .andExpect(jsonPath("$.contractBundleDigest", matchesPattern("^sha256:[a-f0-9]{64}$")))
  ...
  void previousTokenAcceptedDuringRotation() throws Exception {
    mockMvc
        .perform(
            get("/internal/v1/contracts").header(HttpHeaders.AUTHORIZATION, "Bearer " + PREVIOUS))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.service").value("core"));
  }
  ...
  void noPublicApiRouteExposesContractBundle() throws Exception {
    ...
    mockMvc.perform(get("/api/v1/internal/v1/contracts")).andExpect(status().isUnauthorized());
  }
}
