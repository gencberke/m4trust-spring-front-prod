...
class SessionCookiePolicyTest {
  ...
  void productionDefaultsAreHostPrefixedAndSecure() throws IOException {
    ...
    assertEquals(
        "__Host-M4TRUST_SESSION", resolver.getProperty("server.servlet.session.cookie.name"));
    assertEquals("true", resolver.getProperty("server.servlet.session.cookie.secure"));
    assertEquals("true", resolver.getProperty("server.servlet.session.cookie.http-only"));
    assertEquals("lax", resolver.getProperty("server.servlet.session.cookie.same-site"));
    assertEquals("/", resolver.getProperty("server.servlet.session.cookie.path"));
  }
  ...
  void localProfileChangesOnlyNameAndSecureTransport() throws IOException {
    ...
    assertEquals("M4TRUST_SESSION", resolver.getProperty("server.servlet.session.cookie.name"));
    assertEquals("false", resolver.getProperty("server.servlet.session.cookie.secure"));
    assertEquals("true", resolver.getProperty("server.servlet.session.cookie.http-only"));
    assertEquals("lax", resolver.getProperty("server.servlet.session.cookie.same-site"));
    assertEquals("/", resolver.getProperty("server.servlet.session.cookie.path"));
  }
