...
class DealServiceSupersessionTest {
  ...
  void titleChangeLocksDealThenSupersedesBeforePersistingTheDeal() {
    ...
    when(repository.updateBasicFields(any(), any(), any(), any(Long.class), any(), any(), any()))
        .thenReturn(true);
    ...
    order
        ...
        .supersedePending(eq(context), eq(dealId), eq(packageId), any(), eq(NOW));
    order
        .verify(repository)
        .updateBasicFields(
            eq(TENANT), eq(ENTITY), eq(dealId), eq(3L), eq("Changed"), eq("description"), eq(NOW));
  }
  ...
  void descriptionOnlyAndExactTitleUpdatesDoNotSupersede() {
    UUID dealId = UUID.randomUUID();
    OperationContext context = context(RequestedOperation.DEAL_UPDATE);
    stubDeal(context, record(dealId, UUID.randomUUID(), "Original", "description", BUYER, SELLER));
    when(repository.updateBasicFields(any(), any(), any(), any(Long.class), any(), any(), any()))
        .thenReturn(true);
  ...
  void changedAssignmentsSupersedeButExactAssignmentsDoNot() {
    ...
    when(repository.findVisibleByIdForUpdate(TENANT, ENTITY, changedDealId))
        .thenReturn(
            Optional.of(record(changedDealId, changedPackage, "Deal", null, BUYER, SELLER)));
    when(repository.findVisibleByIdForUpdate(TENANT, ENTITY, unchangedDealId))
        .thenReturn(
            Optional.of(record(unchangedDealId, unchangedPackage, "Deal", null, BUYER, SELLER)));
    stubParticipants(changedDealId);
    stubParticipants(unchangedDealId);
    stubRatificationProjection();
    stubFundingProjection();
    when(policy.availableActions(any(), eq(context)))
        .thenReturn(
            new DealAvailableActions(
                true, true, true, true, true, false, false, false, false, false, false, false,
                false, false, false, false, false, false, false, false, false));
    when(policy.isInitiator(any(), eq(context))).thenReturn(true);
    when(repository.updateParties(any(), any(), any(), any(Long.class), any(), any(), any()))
        .thenReturn(true);

    service.updateParties(
        context, changedDealId, parties(BUYER, REPLACEMENT_SELLER), UUID.randomUUID());
    service.updateParties(context, unchangedDealId, parties(BUYER, SELLER), UUID.randomUUID());

    verify(supersessions)
        .supersedePending(eq(context), eq(changedDealId), eq(changedPackage), any(), eq(NOW));
    verify(supersessions, never())
        .supersedePending(eq(context), eq(unchangedDealId), eq(unchangedPackage), any(), any());
  }
  ...
  void withdrawalLocksDealSupersedesCurrentPackageThenCancels() {
    ...
    order
        .verify(repository)
        .updateStatus(TENANT, ENTITY, dealId, DealStatus.DRAFT, DealStatus.CANCELLED, 3, NOW);
  }

  private void stubDeal(OperationContext context, DealRecord record) {
  ...
  private void stubFundingProjection() {
    when(fundingProjections.summarize(any(), anyBoolean(), anyBoolean()))
        .thenReturn(
            new FundingProjectionPort.Summary(
                "NOT_CONFIGURED", null, null, null, false, false, false));
    ...
    when(caseworkProjections.forActor(any()))
        .thenReturn(CaseworkDealProjectionPort.ActorSummary.hidden());
  }

  /**
   * The current-package pointer is always set in these fixtures, so the projection port must
   * resolve it or DealService's invariant check throws.
   */
  private void stubRatificationProjection() {
    when(ratificationProjections.findCurrentPackage(any(), any(), any(), any()))
        .thenReturn(
            Optional.of(
                new RatificationPackageProjectionPort.CurrentPackage(
                    UUID.randomUUID(),
                    0,
                    "PENDING",
                    "a".repeat(64),
                    new RatificationPackageProjectionPort.Snapshot(
                        1,
                        "deal",
                        "DL-1",
                        "Deal",
                        ...
                        new RatificationPackageProjectionPort.RuleSet(
                            UUID.randomUUID().toString(), 1, List.of()),
                        new RatificationPackageProjectionPort.Terms(1, "TRY"),
                        new RatificationPackageProjectionPort.Document(
  ...
  private void stubParticipants(UUID dealId) {
    when(repository.findParticipants(dealId))
        .thenReturn(
            List.of(
                new DealParticipantRecord(BUYER, TENANT, NOW),
                new DealParticipantRecord(SELLER, TENANT, NOW),
                new DealParticipantRecord(REPLACEMENT_SELLER, TENANT, NOW)));
    when(legalEntities.findLegalNames(any()))
        .thenReturn(Map.of(BUYER, "Buyer", SELLER, "Seller", REPLACEMENT_SELLER, "Replacement"));
  }

  private static DealRecord record(
      UUID dealId, UUID packageId, String title, String description, UUID buyer, UUID seller) {
