...
@Service
public class VideoAnalysisEvidenceInputService implements VideoAnalysisEvidenceInputPort {

  private final EvidenceSubmissionRepository evidenceRepository;
  private final FulfillmentObjectStorage storage;

  public VideoAnalysisEvidenceInputService(
      EvidenceSubmissionRepository evidenceRepository, FulfillmentObjectStorage storage) {
  ...
  @Override
  public FulfillmentObjectStorage.DirectDownload mintVersionPinnedDownload(
  ...
  private boolean isSupportedAnalysisInput(EvidenceSubmission.EvidenceSubmissionRecord submission) {
    return switch (submission.evidenceType()) {
      ...
      case PHOTO ->
          submission.mediaType() == EvidenceMediaType.IMAGE_JPEG
