...
class CaseworkVideoAnalysisLockAdapter implements CaseworkSourcePorts.VideoAnalysisJobs {
  ...
  public List<CaseworkSourcePorts.PinnedVideoResult> lockSuccessfulResults(
      UUID dealId, List<UUID> evidenceSubmissionIds) {
    List<UUID> orderedEvidenceIds =
        evidenceSubmissionIds.stream().sorted(Comparator.naturalOrder()).toList();
    ...
    for (UUID evidenceSubmissionId : orderedEvidenceIds) {
      ...
      lockedJobs.stream()
          ...
          .flatMap(
              job ->
                  videoAnalysisJobs
                      .findResultIdByJobId(job.id())
                      .map(
                          resultId ->
                              new CaseworkSourcePorts.PinnedVideoResult(
