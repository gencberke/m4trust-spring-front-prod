...
import com.m4trust.coreapi.organization.domain.IdentityMemberProjectionPort.IdentityMemberProjection;
...
@Service
class LegalEntityService implements CurrentMembershipQueryPort, InvitationLegalEntityQueryPort {
  ...
  @Transactional
  LegalEntity create(
      UUID authenticatedUserId, CreateLegalEntityRequest request, UUID correlationId) {
    UUID tenantId =
        repository
            .findTenantIdForUser(authenticatedUserId)
            .orElseThrow(() -> new IllegalStateException("Authenticated user has no tenant"));
    UUID legalEntityId = UUID.randomUUID();
    UUID membershipId = UUID.randomUUID();
    Instant occurredAt = clock.instant();

    repository.insertLegalEntity(
        legalEntityId, tenantId, request.legalName(), request.registrationNumber());
    repository.insertMembership(
        membershipId, tenantId, legalEntityId, authenticatedUserId, LegalEntityRole.ADMIN);
    ...
    return new LegalEntity(legalEntityId, request.legalName(), request.registrationNumber());
  }

  @Override
  @Transactional(readOnly = true)
  public List<LegalEntityMembership> findMemberships(UUID authenticatedUserId) {
    return List.copyOf(repository.findMemberships(authenticatedUserId));
  }

  @Override
  @Transactional(readOnly = true)
  public java.util.Optional<InvitationLegalEntityMembership> findCurrentMembership(
  ...
  LegalEntity get(OperationContext context) {
    ...
    return repository
        .findLegalEntity(context.tenantId(), context.activeLegalEntityId())
  ...
  LegalEntityMemberList listMembers(OperationContext context) {
    ...
    List<OrganizationRepository.MemberAssignment> assignments =
        repository.findMemberAssignments(context.tenantId(), context.activeLegalEntityId());
    Map<UUID, IdentityMemberProjection> identities =
        identityProjections.findByIds(
            assignments.stream().map(OrganizationRepository.MemberAssignment::userId).toList());
    List<LegalEntityMember> members =
        assignments.stream().map(assignment -> toMember(assignment, identities)).toList();
  ...
  private LegalEntityMember toMember(
    ...
    if (identity == null) {
      throw new IllegalStateException("Membership identity projection is unavailable");
    }
    return new LegalEntityMember(
        identity.userId(), identity.email(), identity.displayName(), assignment.role());
  }
