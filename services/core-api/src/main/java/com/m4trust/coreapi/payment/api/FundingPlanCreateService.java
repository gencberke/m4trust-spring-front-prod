...
class FundingPlanCreateService {
  ...
  FundingPlanDetailView create(
    ...
    IdempotencyRequest idempotencyRequest =
        idempotencyRequest(context, dealId, expectedVersion, idempotencyKey);
    FundingPlanDetailView result =
        transactions.execute(
            status ->
                createInTransaction(
                    context, dealId, expectedVersion, idempotencyRequest, correlationId));
    if (result == null) {
  ...
  private FundingPlanDetailView createInTransaction(
      OperationContext context,
      UUID dealId,
      long expectedVersion,
      IdempotencyRequest idempotencyRequest,
    ...
    FundingSourcePorts.Target target =
        deals
            .lockVisibleForCreate(context, dealId)
    ...
    FundingPlan plan =
        FundingPlan.create(
            ...
            target.tenantId(),
            target.ratifiedAmountMinor(),
            target.ratifiedCurrency(),
            now);
    FundingUnit unit =
        FundingUnit.create(
            UUID.randomUUID(),
    ...
    return FundingProjection.plan(
        plan.toRecord(), unit.toRecord(), null, buyerAdmin, true, mode.resolve());
  }

  private FundingPlanDetailView replay(
      OperationContext context,
      FundingSourcePorts.Target target,
  ...
  private AuditRecord auditRecord(
      OperationContext context, UUID planId, String action, UUID correlationId, Instant now) {
  ...
  private IdempotencyRequest idempotencyRequest(
      OperationContext context, UUID dealId, long expectedVersion, UUID key) {
    try {
