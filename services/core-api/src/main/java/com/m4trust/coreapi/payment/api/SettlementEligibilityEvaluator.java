...
class SettlementEligibilityEvaluator {
  ...
  SettlementEligibilityEvaluator(
      FundingRepository funding, PaymentProviderModeResolver modeResolver, Clock clock) {
  ...
  record Context(
      ...
      FundingRecords.UnitRecord fundingUnit,
  ...
  Evaluation evaluate(Context input, boolean buyerAdmin) {
    ...
    Instant eligibleAt =
        disputeWindowDays != null && completedAt != null
            ? SettlementProjection.releaseEligibleAt(completedAt, disputeWindowDays)
            : null;
    boolean funded =
        input.fundingUnit() != null && input.fundingUnit().status() == FundingUnitStatus.FUNDED;
    boolean fulfillmentCompleted =
    ...
    SettlementStatus projected =
        projectedStatus(input.settlement(), settlementReady, input.operation());
    ...
    return new Evaluation(
        projected, disputeWindowDays, eligibleAt, settlementReady, canRequest, canReconcile);
  }
