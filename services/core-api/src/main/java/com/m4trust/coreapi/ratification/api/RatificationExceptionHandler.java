...
/**
 * Maps ratification service exceptions to the exact Problem Details codes frozen in the OpenAPI
 * contract (paths near line 797, error responses near lines 1690-1745). Non-disclosing 404s never
 * distinguish a missing package from one hidden by Deal visibility.
 */
...
class RatificationExceptionHandler {
  ...
  ResponseEntity<ProblemDetail> createInvalidDisputeWindow(HttpServletRequest request) {
    return validation(
        request, "disputeWindowDays", FieldErrorCode.INVALID, "The value is invalid.");
  }
  ...
  ResponseEntity<ProblemDetail> createInvalidEvidencePolicy(HttpServletRequest request) {
    return validation(
        request, "evidencePolicy", FieldErrorCode.INVALID_ENUM, "The value is invalid.");
  }

  @ExceptionHandler(RatificationPackageCreateService.EvidencePolicyRequiresDisputeWindow.class)
  ResponseEntity<ProblemDetail> createEvidencePolicyRequiresDisputeWindow(
      HttpServletRequest request) {
    return validation(
        request, "disputeWindowDays", FieldErrorCode.REQUIRED, "This field is required.");
  }
  ...
  ResponseEntity<ProblemDetail> packageNotFound(HttpServletRequest request) {
    return response(
        request,
        HttpStatus.NOT_FOUND,
        "ratification-package-not-found",
        "Ratification package not found",
        "The ratification package was not found.",
  ...
  private ResponseEntity<ProblemDetail> validation(
      HttpServletRequest request, String field, FieldErrorCode code, String message) {
