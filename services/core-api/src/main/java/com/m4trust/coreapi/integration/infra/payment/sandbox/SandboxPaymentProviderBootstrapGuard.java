 ...
 *       active profile.
 *   <li>sandbox bean present + {@code staging} active without {@code staging-simulated} also active
 *       — fails (a stray {@code local-sandbox} left active alongside plain {@code staging} still
 *       fails; only the explicit {@code staging}+{@code staging-simulated} pair is allowed).
 *   <li>{@code staging-simulated} + {@code production} together — always fails, independent of
 *       sandbox bean presence.
 * </ul>
 */
@Component
