...
/**
 * Reuses the same wrapper/actor-aware projection {@link RatificationPackageReadService} builds for
 * the dedicated ratification endpoints so the Deal detail's embedded package never drifts from what
 * those endpoints report.
 */
@Service
public class RatificationPackageProjectionService implements RatificationPackageProjectionPort {

  private final RatificationRepository packages;
  private final RatificationPackageReadService reads;

  public RatificationPackageProjectionService(
      RatificationRepository packages, RatificationPackageReadService reads) {
  ...
  public Optional<CurrentPackage> findCurrentPackage(
    ...
    RatificationPackageRecord record =
        packages
            .findByDealAndId(dealId, packageId)
            .orElseThrow(
                () ->
                    new IllegalStateException("Deal current ratification package is unavailable"));
  ...
  private static CurrentPackage toPort(RatificationPackageDetail detail) {
    ...
    return new CurrentPackage(
        ...
        new Snapshot(
            ...
            new RuleSet(
                ...
                snapshot.ruleSet().rules().stream()
                    .map(RatificationPackageProjectionService::rule)
                    .toList()),
            new Terms(
                snapshot.commercialTerms().amountMinor(), snapshot.commercialTerms().currency()),
            new Document(
        ...
        new AvailableActions(
            detail.availableActions().canApprove(), detail.availableActions().canReject()),
        detail.createdAt());
  }
