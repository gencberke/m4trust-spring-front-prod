...
/**
 * Implements the projection port {@code deal.DealService} consumes for its embedded funding
 * summary.
 */
@Service
class FundingProjectionService implements FundingProjectionPort {
  ...
  public Summary summarize(UUID dealId, boolean dealActive, boolean callerIsBuyerAdmin) {
    ...
    FundingRecords.PlanRecord plan = planRecord.get();
    ...
    boolean canInitiate =
        callerIsBuyerAdmin && dealActive && unit.status() != FundingUnitStatus.FUNDED && !inFlight;
    boolean canReconcile =
        callerIsBuyerAdmin
            && currentOperation != null
            && currentOperation.status() == PaymentOperationStatus.UNCONFIRMED;
    return new Summary(
        FundingProjection.dealFundingStatus(unit.status()),
        plan.id(),
        plan.amountMinor(),
        plan.currency(),
        false,
