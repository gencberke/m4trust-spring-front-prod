package com.m4trust.coreapi.ratification.infra.adapter;
...
import com.m4trust.coreapi.ratification.infra.persistence.RatificationRepository;
...
/**
 * Settlement-owned ratification read boundary that accepts schema v2/v3 snapshots with
 * disputeWindowDays without weakening the dedicated package read path.
 */
@Service
class SettlementRatificationSourceAdapter implements SettlementSourcePorts.RatificationTarget {
  ...
  public Optional<SettlementSourcePorts.RatificationSnapshot> findForProjection(
      UUID dealId, UUID packageId) {
            ...
            record -> {
              ...
              try {
                ...
                if ((schemaVersion == 2 || schemaVersion == 3)
                    && raw.hasNonNull("disputeWindowDays")) {
                  disputeWindowDays = raw.get("disputeWindowDays").asInt();
                }
                return new SettlementSourcePorts.RatificationSnapshot(
                    schemaVersion, record.status().name(), disputeWindowDays);
              } catch (Exception exception) {
                throw new IllegalStateException(
                    "Ratification snapshot is unavailable for settlement", exception);
              }
            });
  }
}
