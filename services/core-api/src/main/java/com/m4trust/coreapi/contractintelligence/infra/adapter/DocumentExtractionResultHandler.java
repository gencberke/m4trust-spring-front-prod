...
final class DocumentExtractionResultHandler implements AiTerminalResultHandler {
  ...
  DocumentExtractionResultHandler(
      ObjectMapper mapper,
  ...
  private void apply(JsonNode event, String eventType) {
    ...
    AnalysisRepository.AnalysisJob job =
        repository.findByIdForUpdate(jobId).orElseThrow(IntegrationViolation::new);
  ...
  private void complete(JsonNode event, AnalysisRepository.AnalysisJob job, Instant now) {
    ...
    try {
      repository.insertResult(
          UUID.randomUUID(),
          job.id(),
          event.get("schemaVersion").asString(),
          mapper.writeValueAsString(payload.get("result")),
          now);
    } catch (Exception exception) {
      throw new IllegalStateException("Canonical result cannot be persisted", exception);
    }
    repository.complete(
        job, job.processingStartedAt() == null ? now : job.processingStartedAt(), now);
  ...
  private void fail(JsonNode event, AnalysisRepository.AnalysisJob job, Instant now) {
    ...
    repository.fail(
        job, now, error.path("code").asString(), error.path("retryRecommended").asBoolean());
  ...
  private void appendAudit(
      AnalysisRepository.AnalysisJob job, JsonNode event, String action, Instant now) {
    audit.append(
        new AuditRecord(
            UUID.randomUUID(),
            job.tenantId(),
  ...
  private static boolean terminal(AnalysisJobStatus status) {
    return status == AnalysisJobStatus.REVIEW_REQUIRED
