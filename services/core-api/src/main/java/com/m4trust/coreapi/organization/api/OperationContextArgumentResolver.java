...
@Component
class OperationContextArgumentResolver implements HandlerMethodArgumentResolver {
  ...
  public boolean supportsParameter(MethodParameter parameter) {
    return parameter.getParameterType() == OperationContext.class
        && parameter.hasParameterAnnotation(ResolvedOperationContext.class);
  }
  ...
  public Object resolveArgument(
    ...
    if (authentication == null || !authentication.isAuthenticated()) {
      throw new IllegalStateException("Operation context requires authentication");
    }

    ResolvedOperationContext annotation =
        parameter.getParameterAnnotation(ResolvedOperationContext.class);
    if (annotation == null) {
      throw new IllegalStateException("Resolved operation annotation is unavailable");
    }
    @SuppressWarnings("unchecked")
    Map<String, String> pathVariables =
        (Map<String, String>) request.getAttribute(HandlerMapping.URI_TEMPLATE_VARIABLES_ATTRIBUTE);
    boolean pathMatchRequired = !annotation.legalEntityPathVariable().isBlank();
    String requestedLegalEntityId =
        !pathMatchRequired || pathVariables == null
            ? null
            : pathVariables.get(annotation.legalEntityPathVariable());
