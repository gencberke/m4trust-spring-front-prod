...
class SettlementProjectionService implements SettlementProjectionPort {
  ...
  public Summary summarize(UUID dealId, boolean dealActive, boolean callerIsBuyerAdmin) {
    ...
    SettlementRecords.SettlementRecord settlement = settlements.findByDealId(dealId).orElse(null);
    if (settlement == null) {
      return empty();
    }
    ...
    SettlementEligibilityEvaluator.Evaluation evaluation =
        eligibility.evaluate(context, callerIsBuyerAdmin);
    return new Summary(
        settlement.id(),
  ...
  private SettlementEligibilityEvaluator.Context loadContext(
      ...
      SettlementRecords.ReleaseOperationRecord operation) {
    return deals
        .findForProjection(dealId)
        .map(
            deal -> {
              SettlementSourcePorts.FulfillmentSnapshot fulfillment =
                  fulfillments.findForProjection(dealId).orElse(null);
              SettlementSourcePorts.RatificationSnapshot ratification =
                  deal.ratifiedPackageId() == null
                      ? null
                      : ratifications
                          .findForProjection(dealId, deal.ratifiedPackageId())
                          .orElse(null);
              FundingRecords.PlanRecord plan = funding.findPlanByDeal(dealId).orElse(null);
