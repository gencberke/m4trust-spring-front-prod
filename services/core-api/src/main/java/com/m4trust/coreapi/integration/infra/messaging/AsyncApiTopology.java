...
class AsyncApiTopology {
  ...
  @Bean
  Queue documentExtractionQueue() {
    return durableWorkQueue(DOCUMENT_EXTRACTION_QUEUE);
  }

  @Bean
  Queue videoAnalysisQueue() {
    return durableWorkQueue(VIDEO_ANALYSIS_QUEUE);
  }

  @Bean
  Queue cancellationQueue() {
    return durableWorkQueue(CANCELLATION_QUEUE);
  }

  @Bean
  Queue resultsQueue() {
    return durableWorkQueue(RESULTS_QUEUE);
  }
  ...
  @Bean
  Binding documentExtractionBinding(
      @Qualifier("documentExtractionQueue") Queue queue,
  ...
  Binding videoAnalysisBinding(
      @Qualifier("videoAnalysisQueue") Queue queue,
      @Qualifier("commandsExchange") TopicExchange exchange) {
