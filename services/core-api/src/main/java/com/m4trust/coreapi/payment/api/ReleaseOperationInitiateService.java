...
class ReleaseOperationInitiateService {
  ...
  ReleaseOperationInitiateService(
  ...
  ReleaseOperationView initiate(
      OperationContext context,
      UUID dealId,
      long expectedDealVersion,
      long expectedSettlementVersion,
      long expectedFulfillmentVersion,
      long expectedFundingUnitVersion,
      UUID idempotencyKey,
      UUID correlationId) {
  ...
  private ReleaseOperationView initiateInTransaction(
      OperationContext context,
      UUID dealId,
      long expectedDealVersion,
      long expectedSettlementVersion,
    ...
    FundingRecords.PlanRecord plan =
        funding.findPlanByDeal(dealId).orElseThrow(SettlementExceptions.SettlementNotFound::new);
    FundingRecords.UnitRecord unit =
        funding.findUnitByPlan(plan.id()).orElseThrow(SettlementExceptions.SettlementNotFound::new);
    FundingRecords.UnitLookup unitLookup =
        funding
            .findUnitByIdForUpdate(unit.id())
            .orElseThrow(SettlementExceptions.SettlementNotFound::new);
    ...
    SettlementSourcePorts.RatificationSnapshot ratification =
        deal.ratifiedPackageId() == null
            ? null
            : ratifications
                .findRatifiedPackage(context, dealId, deal.ratifiedPackageId())
                .orElse(null);
    SettlementEligibilityEvaluator.Evaluation evaluation =
        eligibility.evaluate(
            context,
            new SettlementEligibilityEvaluator.Context(
                deal, fulfillment, ratification, unitLookup.unit(), false, settlementRecord, null));
    if (evaluation.disputeWindowDays() == null) {
      throw new SettlementExceptions.ContractualWindowMissing();
    }
    if (!evaluation.releaseEligible()) {
      if (fulfillment.completedAt() != null
          && evaluation.releaseEligibleAt() != null
    ...
    ReleaseOperation operation =
        ReleaseOperation.create(UUID.randomUUID(), settlement.id(), providerKey, now);
    settlements.insertOperation(operation.toRecord());
    settlements.insertDispatch(
        new SettlementRecords.DispatchRecord(
            UUID.randomUUID(),
            operation.id(),
            SettlementRecords.DispatchType.INITIATE,
            providerKey,
            unitLookup.unit().amountMinor(),
            unitLookup.unit().currency(),
            now));
    audit.append(
        auditRecord(context, operation.id(), "RELEASE_OPERATION_INITIATED", correlationId, now));
