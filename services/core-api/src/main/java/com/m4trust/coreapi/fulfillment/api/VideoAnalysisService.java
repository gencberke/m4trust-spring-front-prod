...
public class VideoAnalysisService {
  ...
  public VideoAnalysisService(
      ...
      VideoAnalysisEvidenceInputPort evidenceInputs,
      IdempotencyService idempotency,
      VideoAnalysisCommandEnqueuePort commandEnqueue,
  ...
  public VideoAnalysisDetail get(OperationContext context, UUID dealId, UUID evidenceSubmissionId) {
  ...
  public VideoAnalysisDetail request(
      OperationContext context,
      UUID dealId,
      UUID evidenceSubmissionId,
      RequestVideoAnalysisRequest request,
      UUID idempotencyKey,
      UUID correlationId) {
    requireOperation(context, RequestedOperation.VIDEO_ANALYSIS_REQUEST);
    IdempotencyRequest idempotencyRequest =
        idempotencyRequest(context, dealId, evidenceSubmissionId, request, idempotencyKey);
    ...
    ResolvedEvidence preflightEvidence =
        resolveEligibleEvidence(context, dealId, evidenceSubmissionId);
    ...
    VideoAnalysisEvidenceInputPort.VerifiedSnapshot preflightSnapshot =
        preflightEvidence.snapshot();
    FulfillmentObjectStorage.DirectDownload download =
        evidenceInputs.mintVersionPinnedDownload(preflightSnapshot);
  ...
  private VideoAnalysisDetail requestInTransaction(
      OperationContext context,
      UUID dealId,
      UUID evidenceSubmissionId,
      UUID idempotencyKey,
    ...
    ResolvedEvidence lockedEvidence =
        resolveEligibleEvidenceLocked(target, dealId, evidenceSubmissionId);
    ...
    UUID predecessorJobId =
        repository
            .findLatestByEvidenceId(evidenceSubmissionId)
    ...
    VideoAnalysisRepository.VideoAnalysisJobRecord queuedJob =
        VideoAnalysisRepository.VideoAnalysisJobRecord.queued(
            jobId,
            target.tenantId(),
            dealId,
            lockedEvidence.fulfillmentId(),
            lockedEvidence.milestoneId(),
            evidenceSubmissionId,
            lockedSnapshot.objectVersion(),
            lockedSnapshot.verifiedSha256(),
            lockedSnapshot.verifiedSizeBytes(),
            lockedSnapshot.mediaType().value(),
            lockedSnapshot.fileName(),
            predecessorJobId,
            requestedAt);
    try {
  ...
  private VideoAnalysisDetail projection(ResolvedEvidence resolved, OperationContext context) {
    return repository
        .findLatestByEvidenceId(resolved.evidenceSubmissionId())
        .map(job -> projection(resolved, context, job))
        .orElseGet(() -> notRequested(resolved, context));
  }

  private VideoAnalysisDetail projection(
    ...
    VideoAnalysisFailureSummary failure =
        job.failureCode() == null
            ? null
            : new VideoAnalysisFailureSummary(
                job.failureCode(), Boolean.TRUE.equals(job.retryRecommended()));
    Object result = repository.findResultByJobId(job.id()).map(this::readPublicResult).orElse(null);
    return new VideoAnalysisDetail(
        resolved.evidenceSubmissionId(),
        job.id(),
        publicStatus(job),
        job.requestedAt(),
        job.completedAt(),
  ...
  private Object readPublicResult(String serialized) {
    try {
      ...
      var warnings =
          canonical.has("warnings") ? canonical.get("warnings") : canonical.path("warnings");
      if (!warnings.isArray()) {
  ...
  private VideoAnalysisDetail notRequested(ResolvedEvidence resolved, OperationContext context) {
    return new VideoAnalysisDetail(
        resolved.evidenceSubmissionId(),
        null,
        VideoAnalysisPublicStatus.NOT_REQUESTED,
        null,
        null,
        null,
        null,
  ...
  private VideoAnalysisDetail projectionForJob(
    ...
    ResolvedEvidence resolved =
        resolveEligibleEvidence(context, job.dealId(), job.evidenceSubmissionId());
    return projection(resolved, context, job);
  }

  private VideoAnalysisPublicStatus publicStatus(
      VideoAnalysisRepository.VideoAnalysisJobRecord job) {
  ...
  private ResolvedEvidence resolveEligibleEvidence(
      OperationContext context, UUID dealId, UUID evidenceSubmissionId) {
    FulfillmentSourcePorts.Target target =
        deals.findVisible(context, dealId).orElseThrow(FulfillmentExceptions.DealNotFound::new);
    return resolveEligibleEvidenceRecord(target, evidenceSubmissionId);
  }

  private ResolvedEvidence resolveEligibleEvidenceLocked(
      FulfillmentSourcePorts.Target target, UUID dealId, UUID evidenceSubmissionId) {
    ...
    return resolveEligibleEvidenceRecord(
        fulfillmentRecord, milestoneRecord, evidenceRecord, target);
  }

  private ResolvedEvidence resolveEligibleEvidenceRecord(
      FulfillmentSourcePorts.Target target, UUID evidenceSubmissionId) {
    Fulfillment.FulfillmentRecord fulfillmentRecord =
        fulfillmentRepository
            .findByDealId(target.dealId())
            .orElseThrow(FulfillmentExceptions.FulfillmentNotFound::new);
    Milestone.MilestoneRecord milestoneRecord =
        milestoneRepository
            .findByFulfillmentId(fulfillmentRecord.id())
            .orElseThrow(FulfillmentExceptions.FulfillmentNotFound::new);
    EvidenceSubmission.EvidenceSubmissionRecord evidenceRecord =
        evidenceRepository
            .findById(evidenceSubmissionId)
            .orElseThrow(FulfillmentExceptions.EvidenceNotFound::new);
    return resolveEligibleEvidenceRecord(
        fulfillmentRecord, milestoneRecord, evidenceRecord, target);
  }
  ...
  private boolean isRequestEligible(ResolvedEvidence resolved) {
    ...
    Fulfillment.FulfillmentRecord fulfillmentRecord =
        fulfillmentRepository.findByDealId(target.dealId()).orElse(null);
    if (fulfillmentRecord == null
        || fulfillmentRecord.status() != FulfillmentStatus.REVIEW_REQUIRED) {
      return false;
    }
    Milestone.MilestoneRecord milestoneRecord =
        milestoneRepository.findByFulfillmentId(fulfillmentRecord.id()).orElse(null);
    if (milestoneRecord == null || milestoneRecord.status() != FulfillmentStatus.REVIEW_REQUIRED) {
      return false;
    }
    EvidenceSubmission.EvidenceSubmissionRecord evidenceRecord =
        evidenceRepository.findById(resolved.evidenceSubmissionId()).orElse(null);
    if (evidenceRecord == null
        || EvidenceSubmission.rehydrate(evidenceRecord).status()
  ...
  private static boolean sameImmutableSnapshot(
      VideoAnalysisEvidenceInputPort.VerifiedSnapshot preflight,
  ...
  private static void requireBuyerAdmin(
  ...
  private static FulfillmentExceptions.Conflict activeJobConflict() {
    return new FulfillmentExceptions.Conflict(
        FulfillmentErrorCode.VIDEO_ANALYSIS_ACTIVE_JOB_EXISTS);
  }

  private static FulfillmentExceptions.Conflict alreadyCompletedConflict() {
    return new FulfillmentExceptions.Conflict(
        FulfillmentErrorCode.VIDEO_ANALYSIS_ALREADY_COMPLETED);
  }

  private static FulfillmentExceptions.Conflict notEligibleConflict() {
    return new FulfillmentExceptions.Conflict(
        FulfillmentErrorCode.VIDEO_ANALYSIS_EVIDENCE_NOT_ELIGIBLE);
  }

  private static FulfillmentExceptions.Conflict staleVersionConflict() {
    return new FulfillmentExceptions.Conflict(FulfillmentErrorCode.EVIDENCE_STALE_VERSION);
  }

  private static void requireOperation(
      OperationContext context, RequestedOperation expectedOperation) {
    if (context.requestedOperation() != expectedOperation) {
      throw new IllegalArgumentException(
          "Operation context does not match video analysis use case");
    }
  }
