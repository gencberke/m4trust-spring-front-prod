...
@Component
public final class RatificationSnapshotAssembler {
  ...
  private static final Set<String> PROVENANCE =
      Set.of("EXTRACTED", "REVIEWER_MODIFIED", "MANUALLY_ADDED");
  private static final Set<String> LEGAL_SOURCES =
      Set.of(
          "tbk-6098",
          "kvkk-6698",
          "odeme-hizmetleri-6493",
          "aml-5549",
          "odeme-hizmetleri-yonetmelik",
          "odeme-hizmetleri-tebligi");
  ...
  public RatificationSnapshotAssembler(ObjectMapper json, CanonicalSnapshotHasher hasher) {
    this.json = json;
    this.hasher = hasher;
  }
  ...
  public Result assemble(
    ...
    rules.sort(
        (left, right) ->
            Arrays.compareUnsigned(
                left.ruleReference().getBytes(StandardCharsets.UTF_8),
                right.ruleReference().getBytes(StandardCharsets.UTF_8)));
    ...
    Snapshot snapshot =
        new Snapshot(
            ...
            party(target.seller()),
            new RuleSet(
                uuid(ruleSet.ruleSetVersionId()),
                ruleSet.version(),
                rules.stream().map(this::rule).toList()),
            new Terms(amountMinor, currency),
            new Document(
                uuid(document.documentId()), document.objectVersion(), hex(document.sha256())),
            disputeWindowDays,
            evidencePolicy);
    try {
  ...
  private static void requireIntegralRange(
      JsonNode node, String field, long minimum, long maximum) {
    JsonNode value = node.get(field);
    if (value == null
  ...
  private static void safe(JsonNode value) {
    if (value.isIntegralNumber()
        && (value.bigIntegerValue().compareTo(BigInteger.valueOf(MAX_SAFE_INTEGER)) > 0
            || value.bigIntegerValue().compareTo(BigInteger.valueOf(MAX_SAFE_INTEGER).negate())
                < 0)) {
  ...
  public record Result(Snapshot snapshot, String serializedSnapshot, String contentHash) {}
  ...
  public record Party(String legalEntityId, String legalName) {}

  public record RuleSet(String ruleSetVersionId, long version, List<Rule> rules) {
    public RuleSet {
  ...
  public record Terms(long amountMinor, String currency) {}

  public record Document(String documentId, String objectVersion, String sha256) {}
}
