...
class SettlementController {
  ...
  SettlementController(
      SettlementReadService readService,
      ReleaseOperationInitiateService initiateService,
  ...
  SettlementDetailView getSettlement(
  ...
  ResponseEntity<ReleaseOperationView> requestSettlementRelease(
      @ResolvedOperationContext(RequestedOperation.DEAL_SETTLEMENT_RELEASE)
    ...
    ReleaseOperationView created =
        initiateService.initiate(
            context,
            id(dealId),
            request.expectedDealVersion(),
            request.expectedSettlementVersion(),
            request.expectedFulfillmentVersion(),
            request.expectedFundingUnitVersion(),
            id(idempotencyKey),
            id(correlationId));
  ...
  ReleaseOperationView getReleaseOperation(
  ...
  ResponseEntity<ReleaseOperationView> reconcileReleaseOperation(
      @ResolvedOperationContext(RequestedOperation.RELEASE_OPERATION_RECONCILE)
          OperationContext context,
      @PathVariable String operationId,
      @RequestHeader(value = "Idempotency-Key", required = false) String idempotencyKey,
