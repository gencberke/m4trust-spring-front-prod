...
class RuleSetRepository {
    ...
    void insert(UUID id, UUID dealId, long version, UUID analysisId, UUID extractionId,
        ...
        jdbc.update("INSERT INTO contract_intelligence_rule_set_version (id,deal_id,version,source_analysis_id,source_extraction_result_version_id,created_by_user_id,created_at,previous_rule_set_version_id,rules,excluded_rule_references) VALUES (?,?,?,?,?,?,?, ?,CAST(? AS jsonb),CAST(? AS jsonb))",
                id, dealId, version, analysisId, extractionId, actor, Timestamp.from(createdAt), previousId, rules, excluded);
    }
