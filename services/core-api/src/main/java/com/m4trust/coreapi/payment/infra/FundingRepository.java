...
import com.m4trust.coreapi.payment.domain.FundingRecords.DispatchRecord;
import com.m4trust.coreapi.payment.domain.FundingRecords.OperationLookup;
import com.m4trust.coreapi.payment.domain.FundingRecords.OperationRecord;
import com.m4trust.coreapi.payment.domain.FundingRecords.PlanRecord;
import com.m4trust.coreapi.payment.domain.FundingRecords.UnitLookup;
import com.m4trust.coreapi.payment.domain.FundingRecords.UnitRecord;
...
public class FundingRepository {
  ...
  public FundingRepository(JdbcTemplate jdbc) {
    this.jdbc = jdbc;
  }

  @Transactional(propagation = Propagation.MANDATORY)
  public boolean insertPlanAndUnit(PlanRecord plan, UnitRecord unit) {
    try {
      jdbc.update(
          """
    ...
    jdbc.update(
        ...
        unit.status().name(),
        unit.version(),
        Timestamp.from(unit.createdAt()),
        Timestamp.from(unit.updatedAt()));
    return true;
  }

  public Optional<PlanRecord> findPlanByDeal(UUID dealId) {
    return jdbc
        .query(
            """
  ...
  public Optional<UnitRecord> findUnitByPlan(UUID fundingPlanId) {
    return jdbc
        .query(
            """
  ...
  public Optional<UnitLookup> findUnitById(UUID unitId) {
    return jdbc
        .query(
            """
  ...
  @Transactional(propagation = Propagation.MANDATORY)
  public Optional<UnitLookup> findUnitByIdForUpdate(UUID unitId) {
    return jdbc
        .query(
            """
                SELECT %s FROM funding_unit unit
  ...
  public boolean updateUnitStatus(UnitRecord unit, long previousVersion) {
    return jdbc.update(
            """
  ...
  public Optional<OperationRecord> findInFlightOperation(UUID fundingUnitId) {
    return jdbc
        .query(
            """
                ...
                """,
            this::mapOperationPlain,
            fundingUnitId)
        .stream()
        .findFirst();
  }

  public Optional<OperationRecord> findCurrentOperation(UUID fundingUnitId) {
    return jdbc
        .query(
            """
                SELECT id, funding_unit_id, provider_key, status, provider_reference, created_at, updated_at, version
  ...
  public Optional<OperationLookup> findOperationById(UUID operationId) {
    return jdbc
        .query(
            """
  ...
  @Transactional(propagation = Propagation.MANDATORY)
  public Optional<OperationLookup> findOperationByIdForUpdate(UUID operationId) {
    return jdbc
        .query(
            """
                SELECT %s FROM payment_operation operation
  ...
  public boolean updateOperationStatus(OperationRecord operation, long previousVersion) {
    return jdbc.update(
            """
  ...
  private UnitLookup mapUnitLookup(ResultSet resultSet, int rowNumber) throws SQLException {
    return new UnitLookup(
        mapUnitPlain(resultSet, rowNumber),
        resultSet.getObject("deal_id", UUID.class),
        resultSet.getObject("tenant_id", UUID.class));
  }

  private OperationRecord mapOperationPlain(ResultSet resultSet, int rowNumber)
      throws SQLException {
  ...
  private OperationLookup mapOperationLookup(ResultSet resultSet, int rowNumber)
      throws SQLException {
    return new OperationLookup(
        mapOperationPlain(resultSet, rowNumber),
        resultSet.getObject("deal_id", UUID.class),
        resultSet.getObject("tenant_id", UUID.class),
        resultSet.getLong("unit_amount_minor"),
        resultSet.getString("unit_currency"));
  }
}
