...
public final class CaseworkSourcePorts {
  ...
  public interface VideoAnalysisJobs {
    ...
    /**
     * Locks the requested evidence jobs in deterministic evidence-id order and returns only
     * successful immutable results already committed at lock time.
     */
    List<PinnedVideoResult> lockSuccessfulResults(UUID dealId, List<UUID> evidenceSubmissionIds);
  }

  public record PinnedVideoResult(UUID evidenceSubmissionId, UUID jobId, UUID resultId) {}
