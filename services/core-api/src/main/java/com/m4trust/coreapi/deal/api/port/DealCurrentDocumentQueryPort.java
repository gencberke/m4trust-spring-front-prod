...
/**
 * Deal-owned projection port implemented by the document module. Keeps the Deal detail's
 * currentDocument projection available without the deal module importing the document module (which
 * would create a document&lt;-&gt;deal cycle).
 */
public interface DealCurrentDocumentQueryPort {

  Optional<CurrentDealDocument> findAvailable(UUID documentId);

  record CurrentDealDocument(
      UUID id,
      UUID dealId,
      String fileName,
      String mediaType,
