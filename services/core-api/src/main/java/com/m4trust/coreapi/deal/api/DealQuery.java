...
record DealQuery(int page, int size, DealStatus status, DealSort sort) {

  static DealQuery parse(String pageValue, String sizeValue, String statusValue, String sortValue) {
    ...
    if (statusValue != null) {
      try {
        ...
        throw validation("status", FieldErrorCode.INVALID_ENUM, "Status is not supported.");
      }
    }
    DealSort sort =
        switch (sortValue) {
          ...
          default ->
              throw validation("sort", FieldErrorCode.INVALID_SORT, "Sort is not supported.");
        };
