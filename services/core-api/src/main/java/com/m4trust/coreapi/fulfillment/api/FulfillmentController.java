...
class FulfillmentController {
  ...
  ResponseEntity<FulfillmentDetail> startFulfillment(
      @ResolvedOperationContext(RequestedOperation.FULFILLMENT_START) OperationContext context,
    ...
    FulfillmentDetail result =
        service.startFulfillment(
            context, uuid(dealId), request, uuid(idempotencyKey), uuid(correlationId));
    return ResponseEntity.created(URI.create("/api/v1/deals/" + dealId + "/fulfillment"))
        .body(result);
  }

  @GetMapping("/deals/{dealId}/fulfillment")
  FulfillmentDetail getFulfillment(
      @ResolvedOperationContext(RequestedOperation.FULFILLMENT_READ) OperationContext context,
      @PathVariable String dealId) {
  ...
  ResponseEntity<EvidenceUploadIntent> createEvidenceUploadIntent(
      ...
      @RequestAttribute(CorrelationIdFilter.ATTRIBUTE) String correlationId) {
    EvidenceUploadIntent result =
        service.createEvidenceUploadIntent(context, uuid(dealId), request, uuid(correlationId));
    return ResponseEntity.created(
            URI.create(
                "/api/v1/deals/" + dealId + "/fulfillment/evidence/" + result.evidence().id()))
        .body(result);
  }
  ...
  EvidenceSubmissionProjection finalizeEvidenceUpload(
      ...
      @RequestAttribute(CorrelationIdFilter.ATTRIBUTE) String correlationId) {
    return service.finalizeEvidenceUpload(
        context,
        uuid(dealId),
  ...
  EvidenceSubmissionProjection cancelEvidenceUpload(
      @ResolvedOperationContext(RequestedOperation.EVIDENCE_UPLOAD_CANCEL) OperationContext context,
      @PathVariable String dealId,
      @PathVariable String evidenceSubmissionId,
      @RequestHeader(value = "Idempotency-Key", required = false) String idempotencyKey,
      @Valid @RequestBody CancelEvidenceUploadRequest request,
      @RequestAttribute(CorrelationIdFilter.ATTRIBUTE) String correlationId) {
    return service.cancelEvidenceUpload(
        context,
        uuid(dealId),
        uuid(evidenceSubmissionId),
        request,
  ...
  EvidenceSubmissionProjection acceptEvidence(
      @ResolvedOperationContext(RequestedOperation.EVIDENCE_ACCEPT) OperationContext context,
      @PathVariable String dealId,
      @PathVariable String evidenceSubmissionId,
      @RequestHeader(value = "Idempotency-Key", required = false) String idempotencyKey,
  ...
  EvidenceSubmissionProjection rejectEvidence(
      @ResolvedOperationContext(RequestedOperation.EVIDENCE_REJECT) OperationContext context,
      @PathVariable String dealId,
      @PathVariable String evidenceSubmissionId,
      @RequestHeader(value = "Idempotency-Key", required = false) String idempotencyKey,
      @Valid @RequestBody RejectEvidenceRequest request,
      @RequestAttribute(CorrelationIdFilter.ATTRIBUTE) String correlationId) {
    return service.rejectEvidence(
        context,
        uuid(dealId),
        uuid(evidenceSubmissionId),
        request,
  ...
  FulfillmentDetail acceptWithoutEvidence(
      ...
      @RequestAttribute(CorrelationIdFilter.ATTRIBUTE) String correlationId) {
    return service.acceptWithoutEvidence(
        context, uuid(dealId), request, uuid(idempotencyKey), uuid(correlationId));
  }

  @GetMapping("/deals/{dealId}/fulfillment/evidence/{evidenceSubmissionId}/video-analysis")
  VideoAnalysisDetail getVideoAnalysis(
      @ResolvedOperationContext(RequestedOperation.VIDEO_ANALYSIS_READ) OperationContext context,
      @PathVariable String dealId,
      @PathVariable String evidenceSubmissionId) {
  ...
  ResponseEntity<VideoAnalysisDetail> requestVideoAnalysis(
      @ResolvedOperationContext(RequestedOperation.VIDEO_ANALYSIS_REQUEST) OperationContext context,
      @PathVariable String dealId,
      @PathVariable String evidenceSubmissionId,
