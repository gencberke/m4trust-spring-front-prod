...
public final class ReviewAcceptanceRequestDecoder {
  private static final Set<String> ROOT_FIELDS =
      Set.of("analysisId", "expectedVersion", "decisions");
  private static final Set<String> REFERENCE_DECISION_FIELDS = Set.of("decision", "ruleReference");
  private static final Set<String> EDITED_DECISION_FIELDS =
      Set.of("decision", "ruleReference", "category", "title", "description", "structuredValue");
  private static final Set<String> ADDED_DECISION_FIELDS =
      Set.of("decision", "category", "title", "description", "structuredValue");
  ...
  record EditableRule(
      String category, String title, String description, StructuredValue structuredValue) {}
  ...
  ExtractedRule decodeExtractedRule(JsonNode node) {
    requireObject(node);
    ...
    return new ExtractedRule(
        ...
        title,
        description,
        structuredValue(node.get("structuredValue"), false),
        node.get("confidence").decimalValue(),
        List.copyOf(sources),
        legalBasis(node.get("legalBasis")));
  }

  public RuleSetRule decodeRuleSetRule(JsonNode node) {
    requireObjectWith(
        node,
        Set.of(
            "ruleReference",
    ...
    return new RuleSetRule(
        requiredText(node.get("ruleReference")),
        decision,
        editable.category(),
        editable.title(),
        editable.description(),
        editable.structuredValue(),
        legalBasis(node.get("legalBasis")),
        provenance);
  }
  ...
  private EditableRule editableRule(JsonNode node, boolean requestValue) {
    ...
    return new EditableRule(
        category, title, description, structuredValue(node.get("structuredValue"), requestValue));
  }

  private StructuredValue structuredValue(JsonNode value, boolean finalBoundary) {
    ...
    return switch (value.get("type").asText()) {
      case "TEXT" -> {
        ...
        yield new TextValue("TEXT", requiredText(value.get("value")));
      }
      case "MONEY" -> {
        ...
        if (finalBoundary && !currency.matches("^[A-Z]{3}$"))
          throw validation("structuredValue.currency");
        yield new MoneyValue("MONEY", amount, currency);
      }
      case "PERCENTAGE" -> {
        ...
        if (finalBoundary && (basisPoints < 0 || basisPoints > 10_000))
          throw validation("structuredValue.basisPoints");
        yield new PercentageValue("PERCENTAGE", basisPoints);
      }
      case "DURATION" -> {
        ...
        yield new DurationValue("DURATION", seconds);
      }
      case "DATE" -> {
        requireObjectWith(value, Set.of("type", "value"));
        ...
        if (finalBoundary) {
          try {
            ...
            throw validation("structuredValue.value");
          }
        }
        yield new DateValue("DATE", text);
      }
      case "BOOLEAN" -> {
        requireObjectWith(value, Set.of("type", "value"));
        if (!value.path("value").isBoolean()) throw malformed();
        yield new BooleanValue("BOOLEAN", value.get("value").asBoolean());
      }
      case "QUANTITY" -> {
        ...
        yield new QuantityValue("QUANTITY", value.get("value").decimalValue(), unit);
      }
      default -> throw malformed();
    };
  }

  private LegalBasis legalBasis(JsonNode node) {
    if (node == null || node.isNull()) return null;
    requireObjectWith(node, Set.of("source", "articleNo"));
    return new LegalBasis(requiredText(node.get("source")), requiredText(node.get("articleNo")));
  }
  ...
  static void validateFinalValue(StructuredValue value) {
    if (value instanceof MoneyValue money) {
      ...
      if (percentage.basisPoints() < 0 || percentage.basisPoints() > 10_000)
        throw validation("structuredValue.basisPoints");
    } else if (value instanceof DurationValue duration) {
      if (duration.valueSeconds() < 0) throw validation("structuredValue.valueSeconds");
    } else if (value instanceof DateValue date) {
      try {
        LocalDate.parse(date.value());
      } catch (RuntimeException exception) {
        throw validation("structuredValue.value");
      }
    } else if (value instanceof QuantityValue quantity && quantity.unit().isBlank()) {
      throw validation("structuredValue.unit");
    }
  }

  static void validateFinalRule(ExtractedRule rule) {
  ...
  private static void writeValue(CanonicalWriter writer, StructuredValue value) {
    if (value instanceof TextValue text) {
      writer.field("type", text.type());
      writer.field("value", text.value());
    } else if (value instanceof MoneyValue money) {
      writer.field("type", money.type());
      writer.number("amountMinor", money.amountMinor());
      writer.field("currency", money.currency());
    } else if (value instanceof PercentageValue percentage) {
      writer.field("type", percentage.type());
      writer.number("basisPoints", percentage.basisPoints());
    } else if (value instanceof DurationValue duration) {
      writer.field("type", duration.type());
      writer.number("valueSeconds", duration.valueSeconds());
    } else if (value instanceof DateValue date) {
      writer.field("type", date.type());
      writer.field("value", date.value());
    } else if (value instanceof BooleanValue bool) {
      writer.field("type", bool.type());
      writer.field("value", Boolean.toString(bool.value()));
    } else if (value instanceof QuantityValue quantity) {
      writer.field("type", quantity.type());
      writer.field("value", quantity.value().toPlainString());
      writer.field("unit", quantity.unit());
    }
  }
