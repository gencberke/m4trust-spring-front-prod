...
/** S3-compatible adapter. Provider checksum is used when returned; otherwise bytes are hashed. */
final class S3DocumentObjectStorage implements DocumentObjectStorage {
  ...
  S3DocumentObjectStorage(
      S3Client client, S3Presigner presigner, ObjectStorageProperties properties, Clock clock) {
    this.client = client;
  ...
  public DirectUpload createDirectUpload(String objectKey, String mediaType, long contentLength) {
    if (contentLength <= 0 || contentLength > properties.maxUploadSizeBytes()) {
      throw new IllegalArgumentException("document upload size is outside configured bounds");
    }
    ...
    return new DirectUpload(
        ...
        browserHeaders(signed.signedHeaders()),
  ...
  private DirectDownload createDownload(
      String objectKey, String objectVersion, java.time.Duration ttl) {
  ...
  public VerifiedObject verify(String objectKey) {
    ...
    if (nativeChecksum != null && !nativeChecksum.isBlank()) {
      return new VerifiedObject(head.contentLength(), decodeChecksum(nativeChecksum), version);
    }
    return hashObject(objectKey, version, head.contentLength());
  }

  private VerifiedObject hashObject(String objectKey, String version, long expectedSize) {
    ...
    try (ResponseInputStream<GetObjectResponse> object =
      ...
      throw new IllegalStateException("unable to verify document object", exception);
    }
    if (actualSize != expectedSize) {
      throw new IllegalStateException("object size changed during verification");
    }
    return new VerifiedObject(actualSize, HexFormat.of().formatHex(digest.digest()), version);
  }
