...
public class PaymentDispatchStore {
  ...
  public PaymentDispatchStore(JdbcTemplate jdbc, TransactionTemplate transactions, Clock clock) {
    this.jdbc = jdbc;
  ...
  private static DispatchClaim map(ResultSet resultSet) throws SQLException {
    return new DispatchClaim(
        ...
        FundingRecords.DispatchType.valueOf(resultSet.getString("dispatch_type")),
  ...
  record DispatchClaim(
      ...
      FundingRecords.DispatchType dispatchType,
      UUID providerKey,
      long amountMinor,
      String currency) {}
}
