package com.m4trust.coreapi.document.infra.adapter;

import com.m4trust.coreapi.deal.api.port.DealCurrentDocumentQueryPort;
import com.m4trust.coreapi.deal.api.port.DealCurrentDocumentQueryPort.CurrentDealDocument;
import com.m4trust.coreapi.deal.api.port.DealCurrentDocumentQueryPort.CurrentDealDocumentActions;
...
/**
 * Document-owned implementation of the Deal-owned currentDocument projection port. The Deal module
 * never imports the document module; this class implements the Deal-owned interface instead,
 * keeping the dependency direction document -&gt; deal only.
 */
@Service
class DealCurrentDocumentQueryService implements DealCurrentDocumentQueryPort {
  ...
  private CurrentDealDocument toCurrentDealDocument(Document document) {
    ...
    return new CurrentDealDocument(
        document.id(),
        document.dealId(),
        document.fileName(),
