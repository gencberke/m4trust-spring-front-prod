...
public class FulfillmentRepository {
  ...
  public void insert(Fulfillment.FulfillmentRecord fulfillment) {
    jdbcTemplate.update(
        """
        ...
        fulfillment.version(),
        Timestamp.from(fulfillment.createdAt()),
        Timestamp.from(fulfillment.updatedAt()),
        fulfillment.completedAt() == null ? null : Timestamp.from(fulfillment.completedAt()));
  }

  public Optional<Fulfillment.FulfillmentRecord> findByDealId(UUID dealId) {
    return jdbcTemplate
        .query("SELECT * FROM fulfillment WHERE deal_id = ?", this::mapFulfillment, dealId)
        .stream()
        .findFirst();
  }

  public Optional<Fulfillment.FulfillmentRecord> findByDealIdForUpdate(UUID dealId) {
    return jdbcTemplate
        .query(
            "SELECT * FROM fulfillment WHERE deal_id = ? FOR UPDATE", this::mapFulfillment, dealId)
        .stream()
        .findFirst();
  }

  public Optional<Fulfillment.FulfillmentRecord> findById(UUID fulfillmentId) {
    return jdbcTemplate
        .query("SELECT * FROM fulfillment WHERE id = ?", this::mapFulfillment, fulfillmentId)
        .stream()
        .findFirst();
  }

  public Optional<Fulfillment.FulfillmentRecord> findByIdForUpdate(UUID fulfillmentId) {
    return jdbcTemplate
        .query(
            "SELECT * FROM fulfillment WHERE id = ? FOR UPDATE",
            this::mapFulfillment,
            fulfillmentId)
        .stream()
        .findFirst();
  ...
  public boolean update(Fulfillment.FulfillmentRecord fulfillment, long previousVersion) {
