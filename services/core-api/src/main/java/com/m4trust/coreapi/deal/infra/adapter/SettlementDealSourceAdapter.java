...
class SettlementDealSourceAdapter implements SettlementSourcePorts.DealTarget {
  ...
  SettlementDealSourceAdapter(
      DealRepository deals, RatificationPackageProjectionPort ratificationProjections) {
    this.deals = deals;
  ...
  public Optional<SettlementSourcePorts.DealSnapshot> findVisible(
  ...
  public Optional<SettlementSourcePorts.DealSnapshot> lockVisible(
      OperationContext context, UUID dealId) {
    return deals
  ...
  private SettlementSourcePorts.DealSnapshot snapshotForProjection(Deal deal) {
    return new SettlementSourcePorts.DealSnapshot(
        deal.id(),
  ...
  private SettlementSourcePorts.DealSnapshot snapshot(OperationContext context, Deal deal) {
    ...
    if (currentPackageId != null) {
      String status =
          ratificationProjections.findPackageStatus(deal.id(), currentPackageId).orElse(null);
