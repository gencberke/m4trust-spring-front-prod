...
class PaymentOperationReconcileService {
  ...
  PaymentOperationReconcileService(
  ...
  PaymentOperationView reconcile(
    ...
    IdempotencyRequest idempotencyRequest =
        idempotencyRequest(context, paymentOperationId, expectedVersion, idempotencyKey);
  ...
  private PaymentOperationView reconcileInTransaction(
      OperationContext context,
      UUID paymentOperationId,
      long expectedVersion,
      IdempotencyRequest idempotencyRequest,
    ...
    FundingRecords.OperationLookup lookup =
        funding
            .findOperationByIdForUpdate(paymentOperationId)
    ...
    funding.insertDispatch(
        new FundingRecords.DispatchRecord(
            UUID.randomUUID(),
            operation.id(),
            FundingRecords.DispatchType.RECONCILE,
            operation.providerKey(),
            lookup.unitAmountMinor(),
            lookup.unitCurrency(),
            now));
    audit.append(
        auditRecord(
            context, operation.id(), "PAYMENT_OPERATION_RECONCILE_REQUESTED", correlationId, now));
  ...
  private IdempotencyRequest idempotencyRequest(
      OperationContext context, UUID paymentOperationId, long expectedVersion, UUID key) {
    try {
      ...
      throw new IllegalStateException(
          "Cannot canonicalize payment operation reconcile request", exception);
    }
  }
  ...
  private static void requireOperation(OperationContext context) {
    if (context.requestedOperation() != RequestedOperation.PAYMENT_OPERATION_RECONCILE) {
      throw new IllegalArgumentException(
          "Operation context does not match payment operation reconcile");
    }
  }
}
