...
@Service
class CaseworkVideoAnalysisProjectionAdapter
    implements CaseworkSourcePorts.VideoAnalysisProjection {

  private final VideoAnalysisRepository repository;
  private final ObjectMapper objectMapper;

  CaseworkVideoAnalysisProjectionAdapter(
      VideoAnalysisRepository repository, ObjectMapper objectMapper) {
  ...
  public Object readPinnedPublicResult(UUID jobId, UUID resultId) {
    ...
    String serialized =
        repository
            .findResultByJobId(jobId)
            .orElseThrow(
                () -> new IllegalStateException("Pinned video result payload is unavailable"));
