...
/**
 * Runtime-only settings for the bounded Moka transport. Values are deliberately kept at the
 * integration boundary: credentials must not reach payment domain types, persistence, audit, logs,
 * or public projections.
 */
public record MokaTransportSettings(
    URI baseUri,
    String dealerCode,
    String username,
    String password,
  ...
  public MokaTransportSettings {
    ...
    if (maxRequestBytes < 1 || maxRequestBytes > MAX_SUPPORTED_REQUEST_BYTES) {
      throw new IllegalArgumentException(
          "maxRequestBytes must be between 1 and " + MAX_SUPPORTED_REQUEST_BYTES);
    }
    if (maxResponseBytes < 1 || maxResponseBytes > MAX_SUPPORTED_RESPONSE_BYTES) {
      throw new IllegalArgumentException(
          "maxResponseBytes must be between 1 and " + MAX_SUPPORTED_RESPONSE_BYTES);
    }
  }
