...
import com.m4trust.coreapi.identity.domain.CurrentUserEmailQueryPort;
import com.m4trust.coreapi.organization.domain.InvitationLegalEntityQueryPort;
import com.m4trust.coreapi.organization.domain.InvitationLegalEntityQueryPort.InvitationLegalEntityMembership;
import com.m4trust.coreapi.organization.domain.LegalEntityNotFoundException;
...
class DealInvitationService {
  ...
  private static final String IDEMPOTENCY_OPERATION = "DEAL_INVITATION_CREATE";
  ...
  @Transactional
  DealInvitationProjection create(
      OperationContext context,
      UUID dealId,
      CreateDealInvitationRequest request,
      UUID idempotencyKey,
      UUID correlationId) {
    ...
    IdempotencyClaim claim =
        idempotencyService.claim(
            new IdempotencyRequest(
                ...
                idempotencyKey,
                canonicalHash(context.activeLegalEntityId(), dealId, request.recipientEmail())));
    if (claim.isReplay()) {
      DealInvitationRecord replayed =
          invitationRepository
              .findById(claim.resultReference().id())
              .orElseThrow(
                  () -> new IllegalStateException("Idempotent invitation result is unavailable"));
    ...
    idempotencyService.recordResult(
        claim, new IdempotencyResultReference(IDEMPOTENCY_RESULT, invitation.id()));
    appendAudit(
        context.tenantId(),
        context.authenticatedUserId(),
    ...
    return invitationRepository
        ...
        .orElseThrow(() -> new IllegalStateException("Created invitation is unavailable"));
  }

  @Transactional(readOnly = true)
  DealInvitationPage listForDeal(OperationContext context, UUID dealId, InvitationPageQuery query) {
    requireOperation(context, RequestedOperation.DEAL_INVITATION_LIST_READ);
    Deal deal = loadVisibleDeal(context, dealId);
    requireDraftInitiator(context, deal);
    List<DealInvitationProjection> items =
        invitationRepository.findByDealId(dealId, query.size(), query.offset()).stream()
            .map(record -> toInitiatorProjection(record, context))
            .toList();
    long total = invitationRepository.countByDealId(dealId);
    return new DealInvitationPage(
        items, query.page(), query.size(), total, totalPages(total, query.size()));
  }

  @Transactional(readOnly = true)
  IncomingDealInvitationPage listIncoming(UUID userId, InvitationPageQuery query) {
    String email = currentEmail(userId);
    List<DealInvitationRecord> records =
        invitationRepository.findPendingForRecipient(email, query.size(), query.offset());
    Map<UUID, String> initiatorNames = initiatorNames(records);
    List<IncomingDealInvitation> items =
        records.stream().map(record -> toIncoming(record, initiatorNames)).toList();
    long total = invitationRepository.countPendingForRecipient(email);
    return new IncomingDealInvitationPage(
        items, query.page(), query.size(), total, totalPages(total, query.size()));
  }

  @Transactional
  IncomingDealInvitation accept(
      UUID userId, UUID invitationId, AcceptDealInvitationRequest request, UUID correlationId) {
    String email = currentEmail(userId);
    DealInvitationRecord current = loadRecipientInvitation(invitationId, email);
    ...
    if (current.status() == DealInvitationStatus.ACCEPTED) {
      if (!request.legalEntityId().equals(current.acceptedLegalEntityId())) {
        throw new DealInvitationAcceptedByOtherEntityException();
      }
      return toIncoming(current, initiatorNames(List.of(current)));
    }
    Instant now = clock.instant();
    DealInvitation.rehydrate(current)
        .accept(membership.legalEntityId(), membership.tenantId(), request.version(), now);
    boolean accepted =
        invitationRepository.acceptPending(
            invitationId,
            email,
            request.version(),
            ...
            now);
    if (!accepted) {
      return classifyAcceptFailure(userId, email, invitationId, request, membership);
    }
    invitationRepository.insertParticipant(
        current.dealId(),
        current.tenantId(),
        membership.legalEntityId(),
        membership.tenantId(),
    ...
    appendAudit(
        ...
        correlationId,
        now);
    DealInvitationRecord updated = loadRecipientInvitation(invitationId, email);
  ...
  IncomingDealInvitation reject(
      UUID userId,
      UUID invitationId,
      DealInvitationTerminalActionRequest request,
      UUID correlationId) {
    String email = currentEmail(userId);
    DealInvitationRecord current = loadRecipientInvitation(invitationId, email);
    UUID actorTenantId =
        legalEntityQueries
            .findTenantIdForUser(userId)
            .orElseThrow(
                () -> new IllegalStateException("Authenticated user has no technical tenant"));
    Instant now = clock.instant();
    DealInvitation.rehydrate(current).reject(request.version(), now);
    if (!invitationRepository.rejectPending(invitationId, email, request.version(), now)) {
      classifyTerminalFailure(invitationId, email, request.version());
    }
    appendAudit(actorTenantId, userId, null, invitationId, REJECTED, correlationId, now);
    DealInvitationRecord updated = loadRecipientInvitation(invitationId, email);
    return toIncoming(updated, initiatorNames(List.of(updated)));
  }
  ...
  DealInvitationProjection revoke(
    ...
    DealInvitationRecord current =
        invitationRepository
            .findById(invitationId)
            .orElseThrow(DealInvitationNotFoundException::new);
    Deal deal =
        dealRepository
            .findVisibleById(context.tenantId(), context.activeLegalEntityId(), current.dealId())
            .map(Deal::rehydrate)
            .orElseThrow(DealInvitationNotFoundException::new);
    requireDraftInitiator(context, deal);
    Instant now = clock.instant();
    DealInvitation.rehydrate(current).revoke(request.version(), now);
    if (!invitationRepository.revokePending(
        invitationId, context.activeLegalEntityId(), request.version(), now)) {
  ...
  private IncomingDealInvitation classifyAcceptFailure(
      UUID userId,
      String email,
      UUID invitationId,
      AcceptDealInvitationRequest request,
      InvitationLegalEntityMembership membership) {
    DealInvitationRecord latest = loadRecipientInvitation(invitationId, email);
    if (latest.status() == DealInvitationStatus.ACCEPTED) {
      if (membership.legalEntityId().equals(latest.acceptedLegalEntityId())) {
        return toIncoming(latest, initiatorNames(List.of(latest)));
      }
      throw new DealInvitationAcceptedByOtherEntityException();
    }
    requirePendingVersion(latest, request.version());
    throw new IllegalStateException("Atomic invitation accept failed without a visible conflict");
  }

  private void classifyTerminalFailure(
      UUID invitationId, String recipientEmail, long expectedVersion) {
    DealInvitationRecord latest =
        recipientEmail == null
            ? invitationRepository
                .findById(invitationId)
                .orElseThrow(DealInvitationNotFoundException::new)
  ...
  private void requirePendingVersion(DealInvitationRecord invitation, long expectedVersion) {
  ...
  private void requireDraftInitiator(OperationContext context, Deal deal) {
    if (!deal.isInitiatedBy(context.activeLegalEntityId()) || deal.status() != DealStatus.DRAFT) {
      throw new DealInvitationForbiddenException();
    }
  }

  private DealInvitationRecord loadRecipientInvitation(UUID invitationId, String recipientEmail) {
    return invitationRepository
        .findByIdAndRecipient(invitationId, recipientEmail)
        .orElseThrow(DealInvitationNotFoundException::new);
  }

  private DealInvitationProjection toInitiatorProjection(
      DealInvitationRecord invitation, OperationContext context) {
    boolean canRevoke =
        invitation.status() == DealInvitationStatus.PENDING
            && invitation.deal() != null
            && invitation.deal().status() == DealStatus.DRAFT
            && invitation.deal().initiatorLegalEntityId().equals(context.activeLegalEntityId());
    return new DealInvitationProjection(
        invitation.id(),
        invitation.dealId(),
  ...
  private IncomingDealInvitation toIncoming(
      DealInvitationRecord invitation, Map<UUID, String> initiatorNames) {
    DealRecord deal = invitation.deal();
    String initiatorName = initiatorNames.get(deal.initiatorLegalEntityId());
    if (initiatorName == null) {
      throw new IllegalStateException("Invitation initiator projection is unavailable");
    }
    boolean pending = invitation.status() == DealInvitationStatus.PENDING;
    return new IncomingDealInvitation(
        invitation.id(),
        new DealInvitationDeal(deal.id(), deal.reference(), deal.title(), initiatorName),
        invitation.status(),
        invitation.version(),
        invitation.createdAt(),
        invitation.updatedAt(),
  ...
  private Map<UUID, String> initiatorNames(List<DealInvitationRecord> invitations) {
    Set<UUID> ids =
        invitations.stream()
            .map(DealInvitationRecord::deal)
            .map(DealRecord::initiatorLegalEntityId)
  ...
  private String currentEmail(UUID userId) {
    return emailQueries
        .findNormalizedEmail(userId)
        .orElseThrow(() -> new IllegalStateException("Authenticated user email is unavailable"));
  }

  private void appendAudit(
      UUID actorTenantId,
      UUID actorUserId,
      UUID actorLegalEntityId,
      UUID invitationId,
      String action,
      UUID correlationId,
      Instant occurredAt) {
  ...
  private String canonicalHash(UUID activeLegalEntityId, UUID dealId, String recipientEmail) {
    try {
