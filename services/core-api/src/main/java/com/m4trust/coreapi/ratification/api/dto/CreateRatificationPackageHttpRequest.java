...
/**
 * Wire shape of {@code CreateRatificationPackageRequest} (OpenAPI). Initiator exact commercial
 * terms are always supplied explicitly; accepted MONEY rules are suggestions only and are never
 * silently selected by the server.
 */
public record CreateRatificationPackageHttpRequest(
    @NotNull(message = "Expected version is required.")
        @PositiveOrZero(message = "Expected version must not be negative.")
        @Max(
            value = RatificationPackage.MAX_SAFE_INTEGER,
            message = "Expected version is out of range.")
        Long expectedVersion,
    @NotNull(message = "Commercial terms are required.") @Valid CommercialTerms commercialTerms,
    ...
    @Size(max = 32, message = "evidencePolicy is invalid.")
        @Pattern(
            regexp = "REQUIRED|NOT_REQUIRED",
            message = "evidencePolicy must be REQUIRED or NOT_REQUIRED.")
  ...
  public record CommercialTerms(
      @NotNull(message = "amountMinor is required.")
          ...
          @Max(
              value = RatificationPackage.MAX_SAFE_INTEGER,
              message = "amountMinor is out of range.")
          Long amountMinor,
