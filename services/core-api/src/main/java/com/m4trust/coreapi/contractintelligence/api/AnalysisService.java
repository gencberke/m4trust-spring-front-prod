...
import com.m4trust.coreapi.deal.api.port.DealAnalysisProjectionPort;
import com.m4trust.coreapi.deal.api.port.DealAnalysisReadPort;
import com.m4trust.coreapi.document.domain.DocumentAnalysisInputPort;
import com.m4trust.coreapi.document.domain.DocumentAnalysisSupersedePort;
import com.m4trust.coreapi.document.infra.DocumentObjectStorage;
...
class AnalysisService implements DealAnalysisProjectionPort, DocumentAnalysisSupersedePort {
  ...
  AnalysisService(
      ...
      DocumentAnalysisInputPort documentInputs,
  ...
  DealDocumentAnalysis get(OperationContext context, UUID dealId) {
    ...
    DealAnalysisReadPort.AnalysisVisibility visibility =
        dealReads
  ...
  DealDocumentAnalysis request(
      OperationContext context, UUID dealId, UUID idempotencyKey, UUID correlationId) {
    ...
    DealAnalysisReadPort.AnalysisVisibility preflight =
        dealReads
            .findAnalysisVisibility(context, dealId)
            .orElseThrow(AnalysisExceptions.DealNotFound::new);
    requireEligible(
        preflight.initiator(), preflight.acceptsAnalysis(), preflight.currentDocumentId());
    ...
    DocumentObjectStorage.DirectDownload download =
        storage.createAiDirectDownload(input.objectKey(), input.objectVersion());

    return required(
        transactions.execute(
            status ->
                requestInTransaction(
                    context, dealId, idempotencyKey, correlationId, request, input, download)));
  }

  private DealDocumentAnalysis requestInTransaction(
    ...
    DealAnalysisMutationPort.AnalysisTarget target =
        dealMutations
    ...
    try {
      repository.insertQueued(
          AnalysisRepository.AnalysisJob.queued(
              jobId,
              target.owningTenantId(),
              target.dealId(),
              lockedInput.id(),
              lockedInput.objectVersion(),
              lockedInput.sha256(),
              requestedAt));
    ...
    String event =
        eventFactory.create(
            ...
            requestedAt);
    outbox.enqueue(
        DocumentExtractionRequestedEventFactory.EVENT_TYPE,
        "m4trust.ai.commands",
  ...
  public AnalysisSummary summary(UUID currentDocumentId) {
    ...
    return new AnalysisSummary(
        projection.currentDocumentId(),
        projection.status().name(),
        projection.requestedAt(),
        projection.processingStartedAt(),
        projection.completedAt(),
        projection.failedAt(),
        projection.failure() == null
            ? null
            : new Failure(projection.failure().code(), projection.failure().retryRecommended()));
  }
  ...
  @Override
  public void supersedeForDocument(
      UUID dealId,
      UUID documentId,
      UUID tenantId,
      UUID actorUserId,
      ...
      Instant occurredAt) {
    repository
        .lockAndSupersedeDocument(documentId)
        .forEach(
            job ->
                auditAppender.append(
                    new AuditRecord(
  ...
  private DealDocumentAnalysis currentProjection(UUID currentDocumentId) {
    ...
    return repository
        .findLatestForDocument(currentDocumentId)
        .map(this::projection)
  ...
  private DealDocumentAnalysis projection(AnalysisRepository.AnalysisJob job) {
    DealDocumentAnalysis.Failure failure =
        job.failureCode() == null
            ? null
            : new DealDocumentAnalysis.Failure(
    ...
    return new DealDocumentAnalysis(
        job.documentId(),
        job.status(),
        job.requestedAt(),
  ...
  private Object readPublicResult(String serialized) {
    try {
      ...
      return java.util.Map.of(
          "parties",
          objectMapper.treeToValue(complete.get("parties"), Object.class),
          "rules",
          objectMapper.treeToValue(complete.get("rules"), Object.class),
          "deliveryRequirements",
          objectMapper.treeToValue(complete.get("deliveryRequirements"), Object.class),
          "summary",
          objectMapper.treeToValue(complete.get("summary"), Object.class));
  ...
  private static DealDocumentAnalysis notRequested(UUID currentDocumentId) {
    return new DealDocumentAnalysis(
        currentDocumentId, AnalysisJobStatus.NOT_REQUESTED, null, null, null, null, null, null);
  }
  ...
  private DocumentAnalysisInputPort.Input availableInput(UUID documentId) {
    ...
    return documentInputs
        .findAvailable(documentId)
        .orElseThrow(
            () ->
                new AnalysisExceptions.Conflict(
                    ApiErrorCode.DEAL_DOCUMENT_ANALYSIS_DOCUMENT_NOT_AVAILABLE));
  }

  private static boolean sameImmutableInput(
      DocumentAnalysisInputPort.Input preflight, DocumentAnalysisInputPort.Input locked) {
  ...
  private static void requireEligible(
      boolean initiator, boolean acceptsAnalysis, UUID currentDocumentId) {
    if (!initiator) {
