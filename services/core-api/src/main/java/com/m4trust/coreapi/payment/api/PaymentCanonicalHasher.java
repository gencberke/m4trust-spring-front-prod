...
/**
 * RFC 8785 canonical JSON (JCS), followed by UTF-8 SHA-256 lowercase hex; used for HTTP idempotency
 * hashing.
 */
@Component
final class PaymentCanonicalHasher {
  String hash(String json) {
    try {
      byte[] canonicalUtf8 =
          new JsonCanonicalizer(json).getEncodedString().getBytes(StandardCharsets.UTF_8);
