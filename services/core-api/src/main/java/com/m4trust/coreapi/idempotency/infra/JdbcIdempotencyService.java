...
class JdbcIdempotencyService implements IdempotencyService {
  ...
  @Override
  public Optional<IdempotencyResultReference> findCompleted(IdempotencyRequest request) {
    Optional<IdempotencyRecord> existing = repository.find(request);
    if (existing.isEmpty()) {
      return Optional.empty();
    }
    if (!existing.get().canonicalRequestHash().equals(request.canonicalRequestHash())) {
  ...
  public IdempotencyClaim claim(IdempotencyRequest request) {
    ...
    if (repository.insertIfAbsent(recordId, request, Instant.now())) {
      return new IdempotencyClaim(recordId, IdempotencyClaimStatus.CLAIMED, null);
    }
    ...
    if (!existing.canonicalRequestHash().equals(request.canonicalRequestHash())) {
      throw new IdempotencyKeyReusedException();
    }
  ...
  @Override
  @Transactional(propagation = Propagation.MANDATORY)
  public void recordResult(IdempotencyClaim claim, IdempotencyResultReference resultReference) {
