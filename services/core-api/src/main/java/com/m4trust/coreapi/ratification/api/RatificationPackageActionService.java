...
@Service
public class RatificationPackageActionService {
  ...
  public RatificationPackageActionService(
  ...
  private RatificationPackageReadDtos.Detail mutate(
    ...
    RatificationSourcePorts.Target target =
        deals.lockVisibleForCreate(context, dealId).orElseThrow(NotFound::new);
  ...
  private RatificationPackageReadDtos.Detail approveFresh(
    ...
    if (packages
        .findApprovalByPackageAndEntity(packageState.id(), context.activeLegalEntityId())
        .isPresent()) {
      return finish(context, target, record.id(), claim);
    }

    Instant now = clock.instant();
    packages.insertApproval(
        new RatificationRepository.ApprovalRecord(
            ...
            now));
    appendPackageAudit(
        context, packageState.id(), "RATIFICATION_PACKAGE_APPROVED", correlationId, now);

    boolean otherPartyAlreadyApproved =
        packages.listApprovals(packageState.id()).stream()
    ...
    if (otherPartyAlreadyApproved) {
      ...
      appendPackageAudit(
          context, packageState.id(), "RATIFICATION_PACKAGE_RATIFIED", correlationId, now);
      audit.append(
          new AuditRecord(
              UUID.randomUUID(),
              context.tenantId(),
  ...
  private RatificationPackageReadDtos.Detail rejectFresh(
    ...
    update(packageState, request.expectedPackageVersion());
    appendPackageAudit(
        context, packageState.id(), "RATIFICATION_PACKAGE_REJECTED", correlationId, now);
    return finish(context, target, packageState.id(), claim);
  }
  ...
  private RatificationPackageReadDtos.Detail replay(
    ...
    RatificationPackageRecord result =
        packages.findByDealAndId(target.dealId(), reference.id()).orElseThrow(NotFound::new);
  ...
  private RatificationPackageReadDtos.Detail finish(
    ...
    RatificationPackageRecord stored =
        packages.findByDealAndId(target.dealId(), packageId).orElseThrow(NotFound::new);
    return reads.project(context, target, stored);
  }

  private static void requireAuthority(OperationContext context, RatificationPackageRecord record) {
    boolean assignedParty =
        context.activeLegalEntityId().equals(record.buyerLegalEntityId())
  ...
  private enum Action {
    APPROVE(RequestedOperation.DEAL_RATIFICATION_PACKAGE_APPROVE, "RATIFICATION_PACKAGE_APPROVE"),
    REJECT(RequestedOperation.DEAL_RATIFICATION_PACKAGE_REJECT, "RATIFICATION_PACKAGE_REJECT");
