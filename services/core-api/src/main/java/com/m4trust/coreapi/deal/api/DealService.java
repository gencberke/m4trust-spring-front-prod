...
import com.m4trust.coreapi.casework.domain.CaseworkDealProjectionPort;
...
import com.m4trust.coreapi.fulfillment.domain.port.FulfillmentProjectionPort;
import com.m4trust.coreapi.organization.domain.InvitationLegalEntityQueryPort;
...
import com.m4trust.coreapi.payment.domain.FundingProjectionPort;
import com.m4trust.coreapi.payment.domain.SettlementProjectionPort;
import com.m4trust.coreapi.ratification.domain.RatificationPackageProjectionPort;
import com.m4trust.coreapi.ratification.domain.RatificationSupersessionPort;
...
class DealService {
  ...
  @Transactional
  DealDetail create(OperationContext context, CreateDealRequest request, UUID correlationId) {
  ...
  DealPage list(OperationContext context, DealQuery query) {
    ...
    List<DealSummary> items =
        repository
            ...
            .toList();
    long totalElements =
        repository.countVisible(context.tenantId(), context.activeLegalEntityId(), query.status());
    long pageCount = totalElements / query.size() + (totalElements % query.size() == 0 ? 0 : 1);
    int totalPages = Math.toIntExact(pageCount);
    return new DealPage(items, query.page(), query.size(), totalElements, totalPages);
  }
  ...
  @Transactional
  DealDetail update(
      OperationContext context, UUID dealId, UpdateDealRequest request, UUID correlationId) {
    requireOperation(context, RequestedOperation.DEAL_UPDATE);
    if (!request.descriptionPresent()) {
      throw new DealValidationException(
          "description", FieldErrorCode.REQUIRED, "Description must be provided, and may be null.");
    }
    ...
    deal.updateBasicFields(request.title(), request.description(), request.expectedVersion(), now);
    if (titleChanged) {
      ratificationSupersessions.supersedePending(
          context, deal.id(), deal.currentRatificationPackageId(), correlationId, now);
    }
    boolean updated =
        repository.updateBasicFields(
            context.tenantId(),
            context.activeLegalEntityId(),
            deal.id(),
            currentVersion,
  ...
  @Transactional
  DealDetail cancel(OperationContext context, UUID dealId, UUID correlationId) {
    ...
    if (!repository.updateStatus(
        ...
        currentVersion,
        deal.updatedAt())) {
      throw new DealStateConflictException("Deal changed while cancellation was attempted");
    }
  ...
  DealDetail updateParties(
      OperationContext context, UUID dealId, UpdateDealPartiesRequest request, UUID correlationId) {
    ...
    boolean assignmentsChanged =
        !Objects.equals(deal.buyerLegalEntityId(), request.buyerLegalEntityId())
            || !Objects.equals(deal.sellerLegalEntityId(), request.sellerLegalEntityId());
    deal.assignParties(
        request.buyerLegalEntityId(),
  ...
  private void classifyFailedUpdate(OperationContext context, UUID dealId, long expectedVersion) {
    ...
    throw new IllegalStateException("Atomic Deal update failed without a visible conflict");
  }

  private void classifyFailedPartyUpdate(
      OperationContext context, UUID dealId, long expectedVersion) {
    Deal latest = loadVisible(context, dealId);
    operationPolicy.requireInitiator(latest, context);
    ...
    throw new IllegalStateException("Atomic Deal party update failed without a visible conflict");
  }

  private void validatePartyRequest(UpdateDealPartiesRequest request, UUID dealId) {
    ...
    if (request.buyerLegalEntityId() != null
        && request.buyerLegalEntityId().equals(request.sellerLegalEntityId())) {
      throw new DealValidationException(
          "buyerLegalEntityId",
          FieldErrorCode.MUST_DIFFER,
          "Buyer and seller must be different legal entities.");
    }
    java.util.Set<UUID> participantIds =
        repository.findParticipants(dealId).stream()
            .map(DealParticipantRecord::legalEntityId)
  ...
  private DealFundingSummary fundingSummary(FundingProjectionPort.Summary summary) {
    return new DealFundingSummary(
        summary.fundingStatus(),
        summary.fundingPlanId(),
        summary.amountMinor(),
        summary.currency());
  }
  ...
  /**
   * READY is derived, never persisted: parties, an accepted rule-set, and a current document
   * pointer must exist together (§5 NOT_READY/READY hesabı). The current-package pointer is looked
   * up by id regardless of its terminal status so SUPERSEDED/REJECTED history stays visible.
   */
  private DealRatificationProjection ratification(Deal deal, OperationContext context) {
    ...
    DealRatificationReadiness readiness =
        ready ? DealRatificationReadiness.READY : DealRatificationReadiness.NOT_READY;
  ...
  private DealCurrentDocumentQueryPort.CurrentDealDocument currentDocument(Deal deal) {
    ...
    return currentDocumentQueries
        .findAvailable(currentDocumentId)
        .orElseThrow(() -> new IllegalStateException("Deal current document is unavailable"));
  }

  private DealSummary toSummary(Deal deal, OperationContext context) {
    FundingProjectionPort.Summary fundingSummary =
        fundingProjections.summarize(deal.id(), deal.status() == DealStatus.ACTIVE, false);
    FulfillmentProjectionPort.Summary fulfillmentSummary =
        fulfillmentProjections.summarize(deal.id());
    CaseworkDealProjectionPort.ActorSummary caseworkSummary =
        caseworkProjection(deal, context, fulfillmentSummary);
  ...
  private DealAvailableActions actions(
      Deal deal, OperationContext context, boolean canOpenDispute) {
    DealAvailableActions base = operationPolicy.availableActions(deal, context);
    return new DealAvailableActions(
        base.canUpdate(),
        base.canCancel(),
        base.canCreateInvitation(),
        base.canManageParties(),
        base.canCreateDocumentUploadIntent(),
        base.canRequestAnalysis(),
        base.canReviewExtraction(),
        base.canCreateRatificationPackage(),
        base.canApproveRatification(),
        base.canRejectRatification(),
        base.canCreateFundingPlan(),
        base.canInitiateFunding(),
        base.canReconcilePaymentOperation(),
        base.canStartFulfillment(),
        base.canUploadEvidence(),
        base.canAcceptEvidence(),
        base.canRejectEvidence(),
        base.canAcceptWithoutEvidence(),
        canOpenDispute,
        false,
        false);
  }

  private DealAvailableActions actionsWithAnalysis(
      Deal deal,
      OperationContext context,
    ...
    boolean canCreateRatificationPackage =
        operationPolicy.isInitiator(deal, context)
    ...
    boolean canApproveRatification =
        currentPackage != null && currentPackage.availableActions().canApprove();
    boolean canRejectRatification =
        currentPackage != null && currentPackage.availableActions().canReject();
    String fulfillmentStatus =
        fulfillmentSummary == null || fulfillmentSummary.status() == null
            ? null
            : fulfillmentSummary.status().name();
    ...
    boolean canUploadEvidence =
        isActive
            && deal.sellerLegalEntityId() != null
            && context.activeLegalEntityId().equals(deal.sellerLegalEntityId())
            && ("IN_PROGRESS".equals(fulfillmentStatus)
                || "EVIDENCE_REQUIRED".equals(fulfillmentStatus))
            && fulfillmentSummary != null
            && fulfillmentSummary.evidencePolicy() == EvidencePolicy.REQUIRED;
    boolean canAcceptEvidence =
        isActive
            && context.activeLegalEntityRole()
                == com.m4trust.coreapi.organization.domain.LegalEntityRole.ADMIN
            && deal.buyerLegalEntityId() != null
    ...
    boolean canAcceptWithoutEvidence =
        isActive
            ...
            && context.activeLegalEntityId().equals(deal.buyerLegalEntityId())
            && fulfillmentSummary != null
            && fulfillmentSummary.evidencePolicy() == EvidencePolicy.NOT_REQUIRED
            && fulfillmentSummary.status() == FulfillmentStatus.IN_PROGRESS
    ...
    return new DealAvailableActions(
        ...
        canCreateRatificationPackage,
        canApproveRatification,
        canRejectRatification,
        fundingSummary.canCreateFundingPlan(),
        fundingSummary.canInitiateFunding(),
        ...
        settlementSummary.canRequestRelease(),
        settlementSummary.canReconcileRelease());
  }

  private CaseworkDealProjectionPort.ActorSummary caseworkProjection(
      Deal deal, OperationContext context, FulfillmentProjectionPort.Summary fulfillmentSummary) {
    String fulfillmentStatus =
        fulfillmentSummary == null || fulfillmentSummary.status() == null
            ? null
            : fulfillmentSummary.status().name();
    return caseworkProjections.forActor(
        new CaseworkDealProjectionPort.ActorContext(
            deal.id(),
  ...
  private DealSettlementSummary toSettlementSummary(SettlementProjectionPort.Summary summary) {
    ...
    return new DealSettlementSummary(
        summary.settlementId(), summary.status(), summary.currentReleaseOperationId());
  }
  ...
  private DealAnalysisProjectionPort.AnalysisSummary analysis(Deal deal) {
    ...
    return documentId == null
        ? new DealAnalysisProjectionPort.AnalysisSummary(
            null, "NOT_REQUESTED", null, null, null, null, null)
  ...
  private DealLifecycleProjection lifecycle(
    ...
    UUID documentId = deal.currentDocumentId();
    boolean current =
        documentId != null && currentDocumentQueries.findAvailable(documentId).isPresent();
    ...
    return DealLifecycleProjectionCalculator.calculate(
        ...
        current,
        fundingStatus,
        caseworkSummary.activeDispute() != null,
        fulfillmentStatus);
  }
  ...
  private List<DealParticipant> participants(Deal deal) {
    List<DealParticipantRecord> participantRecords = repository.findParticipants(deal.id());
    java.util.Map<UUID, String> names =
        legalEntityQueries.findLegalNames(
        ...
        .map(
            participant -> {
              ...
              return new DealParticipant(
                  ...
                  partyRoles(deal, participant.legalEntityId()));
            })
        .toList();
  }
  ...
  private DealParty party(UUID legalEntityId, List<DealParticipant> participants) {
    if (legalEntityId == null) {
      return null;
    }
    return participants.stream()
        .filter(participant -> participant.legalEntityId().equals(legalEntityId))
        .findFirst()
        .map(participant -> new DealParty(participant.legalEntityId(), participant.legalName()))
        .orElseThrow(
            () -> new IllegalStateException("Deal party is not represented by a participant"));
  }
