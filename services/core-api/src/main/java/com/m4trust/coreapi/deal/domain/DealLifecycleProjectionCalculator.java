...
public final class DealLifecycleProjectionCalculator {

  private static final Set<String> KNOWN_FULFILLMENT_STATUSES =
      Set.of(
          "NOT_STARTED",
          "IN_PROGRESS",
          "EVIDENCE_REQUIRED",
   ...
   *
   * <p>Later slices extend this central calculator when their independent status modules become
   * available; they do not persist or calculate a separate lifecycle field.
   */
  public static DealLifecycleProjection calculate(DealStatus dealStatus) {
    return calculate(dealStatus, "NOT_REQUESTED", false, "NOT_CONFIGURED");
  }

  public static DealLifecycleProjection calculate(
      DealStatus dealStatus, String analysisStatus, boolean currentDocumentExists) {
  ...
  public static DealLifecycleProjection calculate(
    ...
    if (fulfillmentStatus != null && !KNOWN_FULFILLMENT_STATUSES.contains(fulfillmentStatus)) {
      throw new IllegalArgumentException("Unknown fulfillment status: " + fulfillmentStatus);
    }
    ...
    return switch (dealStatus) {
      ...
      case ACTIVE -> {
        ...
        yield switch (fundingStatus) {
          ...
          default -> throw new IllegalArgumentException("Unknown funding status: " + fundingStatus);
        };
      }
      case DRAFT ->
          !currentDocumentExists
              ...
              : switch (analysisStatus) {
                case "QUEUED", "PROCESSING", "FAILED", "NOT_REQUESTED", "SUPERSEDED" ->
                    DealLifecycleProjection.CONTRACT_ANALYSIS;
