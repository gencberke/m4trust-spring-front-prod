package com.m4trust.coreapi.integration.infra.payment.moka;

import com.m4trust.coreapi.payment.domain.PaymentProviderPort;
...
/**
 * Bounded, provider-neutral Moka HTTP adapter. It neither logs nor returns raw upstream bodies,
 * credentials, CheckKey values, or provider messages.
 */
public final class MokaHttpPaymentProviderAdapter implements PaymentProviderPort {
  ...
  MokaHttpPaymentProviderAdapter(
      MokaTransportSettings settings, HttpClient httpClient, ObjectMapper objectMapper) {
  ...
  private PoolProbeFact probe(String path, String providerKey) {
    ...
    return exchangeProbe(
        path, Map.of("PaymentDealerAuthentication", authentication(), "OtherTrxCode", providerKey));
  }
  ...
  private Map<String, String> authentication() {
    return Map.of(
        ...
        "CheckKey",
            MokaAuthentication.checkKey(
                settings.dealerCode(), settings.username(), settings.password()));
  }
  ...
  ProviderResult parse(byte[] responseBody) {
    try {
      ...
      if (root == null
          || !root.isObject()
          || !root.path("IsSuccessful").isBoolean()
          || !root.path("ResultCode").isTextual()) {
        return unknown();
      }
      String code = root.path("ResultCode").textValue();
      if (!root.path("IsSuccessful").booleanValue()
          && "NOT_FOUND".equals(code)
          && root.path("VirtualPosOrderId").isMissingNode()
          && root.path("OtherTrxCode").isMissingNode()) {
  ...
  private PoolProbeFact exchangeProbe(String path, Map<String, ?> payload) {
    try {
      ...
      if (!root.path("IsSuccessful").booleanValue()
          && "NOT_FOUND".equals(root.path("ResultCode").textValue())
          && root.path("VirtualPosOrderId").isMissingNode()
          && root.path("OtherTrxCode").isMissingNode()) {
      ...
      return root.path("IsSuccessful").booleanValue() && reference != null
          ? PoolProbeFact.accepted(reference)
          : PoolProbeFact.unknown();
    } catch (IOException | InterruptedException exception) {
  ...
  record PoolProbeFact(PoolProbeOutcome outcome, String opaqueReference) {
    static PoolProbeFact accepted(String reference) {
      return new PoolProbeFact(PoolProbeOutcome.ACCEPTED_NOT_FINAL, reference);
    }

    static PoolProbeFact notFound() {
      return new PoolProbeFact(PoolProbeOutcome.NOT_FOUND, null);
    }

    static PoolProbeFact unknown() {
      return new PoolProbeFact(PoolProbeOutcome.UNKNOWN, null);
    }
  }

  enum PoolProbeOutcome {
    ACCEPTED_NOT_FINAL,
    NOT_FOUND,
    UNKNOWN
  }
