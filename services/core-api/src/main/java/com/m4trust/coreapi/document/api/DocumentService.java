...
import com.m4trust.coreapi.deal.api.port.DealDocumentMutationPort;
import com.m4trust.coreapi.deal.api.port.DealDocumentMutationPort.LockedDealDocumentTarget;
import com.m4trust.coreapi.deal.api.port.DealDocumentReadPort;
import com.m4trust.coreapi.deal.api.port.DealDocumentReadPort.DealDocumentVisibility;
...
class DocumentService {
  ...
  DocumentService(
      ...
      Clock clock,
      DocumentAnalysisSupersedePort analysisSupersedes,
  ...
  DocumentUploadIntent createIntent(
      OperationContext context,
      UUID dealId,
      CreateDocumentUploadIntentRequest request,
      UUID correlationId) {
    ...
    DocumentObjectStorage.DirectUpload directUpload =
    ...
    return required(
        transactions.execute(
            status -> {
              ...
              return new DocumentUploadIntent(
  ...
  AvailableDealDocument finalizeUpload(
      OperationContext context,
      UUID documentId,
      FinalizeDocumentUploadRequest request,
      UUID idempotencyKey,
      UUID correlationId) {
    requireOperation(context, RequestedOperation.DOCUMENT_UPLOAD_FINALIZE);
    IdempotencyRequest idempotencyRequest =
        idempotencyRequest(context, documentId, request, idempotencyKey);
    ...
    requireUploadSize(request.sizeBytes());
    DocumentRecord preflight =
        repository.findById(documentId).orElseThrow(DocumentExceptions.NotFound::new);
        ...
        transactions.execute(
            status ->
                finalizeInTransaction(
                    context, documentId, request, idempotencyRequest, correlationId, verified)));
  }

  private AvailableDealDocument finalizeInTransaction(
      OperationContext context,
      UUID documentId,
      FinalizeDocumentUploadRequest request,
      IdempotencyRequest idempotencyRequest,
    ...
    LockedDealDocumentTarget target =
        requireFinalizeMutationTarget(lockedTarget(context, current.dealId()));
    ...
    if (previousCurrentDocumentId != null) {
      Document previous =
          repository
              .findByIdForUpdate(previousCurrentDocumentId)
              .map(Document::rehydrate)
      ...
      analysisSupersedes.supersedeForDocument(
          target.dealId(),
          previousCurrentDocumentId,
          target.tenantId(),
    ...
    idempotency.recordResult(
        claim, new IdempotencyResultReference(IDEMPOTENCY_RESULT, document.id()));
  ...
  DealDocumentHistory listHistory(OperationContext context, UUID dealId) {
    ...
    DealDocumentVisibility target =
        dealReads.findVisibility(context, dealId).orElseThrow(DocumentExceptions.DealNotFound::new);
    Instant now = clock.instant();
    List<DealDocumentHistoryItem> items =
        repository.findByDealId(dealId).stream()
            .map(Document::rehydrate)
  ...
  DocumentDownloadLink createDownloadLink(OperationContext context, UUID documentId) {
    ...
    DocumentRecord record =
        repository.findById(documentId).orElseThrow(DocumentExceptions.NotFound::new);
    dealReads
        .findVisibility(context, record.dealId())
        .orElseThrow(DocumentExceptions.NotFound::new);
    Document document = Document.rehydrate(record);
    if (document.status() == DocumentStatus.PENDING_UPLOAD || document.objectVersion() == null) {
    ...
    DocumentObjectStorage.DirectDownload download =
        storage.createDirectDownload(document.objectKey(), document.objectVersion());
    return new DocumentDownloadLink(
        document.id(), document.objectVersion(), download.url(), download.expiresAt());
  }

  private DealDocumentHistoryItem toHistoryItem(Document document, boolean initiator, Instant now) {
    if (document.status() == DocumentStatus.PENDING_UPLOAD) {
  ...
  private HistoricalDealDocument historical(Document document) {
    return new HistoricalDealDocument(
        document.id(),
        document.dealId(),
        document.fileName(),
  ...
  private LockedDealDocumentTarget preflightDealTarget(OperationContext context, UUID dealId) {
    return required(transactions.execute(status -> lockedDealTarget(context, dealId)));
  }

  private LockedDealDocumentTarget preflightDocumentTarget(OperationContext context, UUID dealId) {
    return required(transactions.execute(status -> lockedTarget(context, dealId)));
  }

  private LockedDealDocumentTarget lockedDealTarget(OperationContext context, UUID dealId) {
    return dealMutations
        .lockForDocumentMutation(context, dealId)
        .orElseThrow(DocumentExceptions.DealNotFound::new);
  }

  private LockedDealDocumentTarget lockedTarget(OperationContext context, UUID dealId) {
    return dealMutations
        .lockForDocumentMutation(context, dealId)
        .orElseThrow(DocumentExceptions.NotFound::new);
  }

  private LockedDealDocumentTarget requireMutationTarget(LockedDealDocumentTarget target) {
    if (!target.initiator()) {
  ...
  private LockedDealDocumentTarget requireFinalizeMutationTarget(LockedDealDocumentTarget target) {
    if (!target.initiator()) {
      throw new DocumentExceptions.MutationForbidden();
    }
    if (!target.acceptsDocuments()) {
  ...
  private PendingDealDocument pending(Document document, boolean canFinalize) {
    return new PendingDealDocument(
        document.id(),
        document.dealId(),
        document.fileName(),
        document.mediaType().value(),
  ...
  private AvailableDealDocument available(Document document, boolean canDownload) {
    return new AvailableDealDocument(
        document.id(),
        document.dealId(),
        document.fileName(),
        document.mediaType().value(),
  ...
  private AvailableDealDocument replayAvailable(IdempotencyResultReference reference) {
    Document document =
        repository
  ...
  private String canonicalHash(
      UUID activeLegalEntityId, UUID documentId, long sizeBytes, String sha256) {
    try {
  ...
  private IdempotencyRequest idempotencyRequest(
    ...
    return new IdempotencyRequest(
        ...
        idempotencyKey,
        canonicalHash(
            context.activeLegalEntityId(), documentId, request.sizeBytes(), request.sha256()));
  }
