...
class MessagingBrokerBootstrapGuard {
  ...
  MessagingBrokerBootstrapGuard(Environment environment) {
    boolean topologyEnabled =
        environment.getProperty("app.messaging.topology.enabled", Boolean.class, false);
    boolean relayEnabled =
        environment.getProperty("app.messaging.relay.enabled", Boolean.class, false);
  ...
  private static void requirePresent(Environment environment, String property) {
    ...
    if (value == null || value.isBlank()) {
      throw new IllegalStateException(
          "Messaging is enabled but required broker setting is missing or blank: " + property);
    }
  }
  ...
  private static void requireValidPort(String portValue) {
    ...
    try {
      ...
      throw new IllegalStateException(
          "Messaging is enabled but spring.rabbitmq.port is not a valid integer: " + portValue, ex);
    }
    if (port < MIN_TCP_PORT || port > MAX_TCP_PORT) {
      throw new IllegalStateException(
          "Messaging is enabled but spring.rabbitmq.port is outside TCP range 1..65535: " + port);
    }
  }
}
