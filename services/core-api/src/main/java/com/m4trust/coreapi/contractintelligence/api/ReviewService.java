...
import com.m4trust.coreapi.deal.api.port.DealAnalysisMutationPort;
import com.m4trust.coreapi.deal.api.port.DealAnalysisReadPort;
import com.m4trust.coreapi.deal.api.port.DealRuleSetProjectionPort;
...
class ReviewService implements DealRuleSetProjectionPort {
  ...
  ReviewService(
      ...
      DealAnalysisReadPort dealReads,
      DealAnalysisMutationPort dealMutations,
  ...
  Review review(OperationContext context, UUID dealId) {
    ...
    var job =
        analyses
            .findLatestForDocument(visibility.currentDocumentId())
    ...
    return new Review(job.id(), job.documentId(), extractedRules(job.id()));
  }
  ...
  Version accept(
    ...
    IdempotencyRequest idempotencyRequest =
        new IdempotencyRequest(
            ...
            idempotencyKey,
            decoder.canonicalHash(context.activeLegalEntityId(), dealId, request));
    var completed = idempotency.findCompleted(idempotencyRequest);
    if (completed.isPresent()) {
      return versionForDeal(dealId, completed.get().id());
    }
    return transaction.execute(
        status -> acceptLocked(context, dealId, correlationId, idempotencyRequest, request));
  }

  History history(OperationContext context, UUID dealId) {
    require(context, RequestedOperation.DEAL_RULE_SET_VERSION_READ);
    requireVisible(context, dealId);
    return new History(ruleSets.list(dealId).stream().map(this::summary).toList());
  }

  Version version(OperationContext context, UUID dealId, UUID versionId) {
    require(context, RequestedOperation.DEAL_RULE_SET_VERSION_READ);
  ...
  private Version acceptLocked(
      OperationContext context,
      UUID dealId,
      UUID correlationId,
      IdempotencyRequest idempotencyRequest,
      ReviewAcceptanceRequestDecoder.Request request) {
    var target =
        dealMutations
            .lockForReview(context, dealId)
            .orElseThrow(AnalysisExceptions.DealNotFound::new);
    ...
    var job =
        analyses
            .findByIdForUpdate(request.analysisId())
            .filter(
                candidate ->
                    candidate.dealId().equals(dealId)
    ...
    DecisionOutput output =
        new ReviewDecisionAssembler().assemble(extractedRules(job.id()), request.decisions());
    ...
    ruleSets.insert(
        ...
        write(output.rules()),
        write(output.excludedRuleReferences()));
    analyses.accept(job);
    dealMutations.setCurrentRuleSet(dealId, versionId, now);
    audit.append(
        new AuditRecord(
            UUID.randomUUID(),
            target.owningTenantId(),
            context.authenticatedUserId(),
            context.activeLegalEntityId(),
            "RULE_SET_VERSION",
            versionId,
            "EXTRACTION_REVIEW_ACCEPTED",
            correlationId,
            null,
            now));
  ...
  private List<ExtractedRule> extractedRules(UUID analysisId) {
    try {
      JsonNode root =
          json.readTree(
              analyses
                  .findResult(analysisId)
                  .orElseThrow(
                      () -> new AnalysisExceptions.Conflict(ApiErrorCode.DEAL_STATE_CONFLICT)));
      if (!root.path("rules").isArray())
        throw new IllegalStateException("Canonical extraction has no rules");
      List<ExtractedRule> result = new ArrayList<>();
  ...
  private Version versionForDeal(UUID dealId, UUID versionId) {
    RuleSetRepository.Row row =
        ruleSets
            .find(dealId, versionId)
            .orElseThrow(AnalysisExceptions.RuleSetVersionNotFound::new);
    try {
      JsonNode rulesNode = json.readTree(row.rules());
      List<RuleSetRule> finalRules = new ArrayList<>();
  ...
  private Summary summary(RuleSetRepository.Row row) {
    return new Summary(
        row.id(),
        row.version(),
        row.analysisId(),
  ...
  record DecisionOutput(List<RuleSetRule> rules, List<String> excludedRuleReferences) {}
  ...
  static final class ReviewDecisionAssembler {
    DecisionOutput assemble(
        List<ExtractedRule> extracted, List<ReviewAcceptanceRequestDecoder.Decision> decisions) {
      ...
      List<String> excluded = new ArrayList<>();
      for (ExtractedRule original : extracted) {
        if (!extractedReferences.add(original.ruleReference())) throw malformed();
        ReviewAcceptanceRequestDecoder.Decision decision =
            byReference.remove(original.ruleReference());
    ...
    private static RuleSetRule finalRule(
        String reference,
        String decision,
        String category,
        String title,
      ...
      return new RuleSetRule(
          reference, decision, category, title, description, value, legalBasis, provenance);
    }
    ...
    private static String referenceOf(ReviewAcceptanceRequestDecoder.Decision decision) {
      ...
      if (decision instanceof ReviewAcceptanceRequestDecoder.Modified modified)
        return modified.ruleReference();
      if (decision instanceof ReviewAcceptanceRequestDecoder.Excluded excluded)
        return excluded.ruleReference();
      throw malformed();
    }
