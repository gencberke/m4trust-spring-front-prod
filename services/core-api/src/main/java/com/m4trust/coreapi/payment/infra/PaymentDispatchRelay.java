...
@ConditionalOnProperty(
    ...
    name = "enabled",
    havingValue = "true")
public class PaymentDispatchRelay {
  ...
  PaymentDispatchRelay(
      PaymentDispatchStore store,
  ...
  /**
   * Public so integration tests can drive dispatch deterministically instead of waiting on the
   * schedule.
   */
  public void relayOnce() {
    for (PaymentDispatchStore.DispatchClaim claim :
      ...
      try {
        ...
        LOGGER.warn(
            "payment dispatch deferred dispatchId={} operationId={}",
            claim.id(),
  ...
  private PaymentProviderPort.ProviderResult resolveOutsideTransaction(
      PaymentDispatchStore.DispatchClaim claim) {
        ...
        status -> {
          FundingRecords.OperationLookup lookup =
              funding
                  ...
                  .orElseThrow(
                      () ->
                          new IllegalStateException("Dispatched payment operation is unavailable"));
          PaymentOperation operation = PaymentOperation.rehydrate(lookup.operation());
          if (operation.terminal()) {
            // Already resolved by an earlier relay pass (e.g. a reclaim after a crash); idempotent
            // no-op.
            return;
          }
          ...
          switch (result.outcome()) {
            ...
            case UNCONFIRMED, NOT_FOUND -> {
              if (operation.status() != PaymentOperationStatus.UNCONFIRMED) {
                ...
                requireUpdated(
                    funding.updateOperationStatus(operation.toRecord(), previousVersion));
  ...
  private void fundUnit(UUID fundingUnitId, Instant now) {
    FundingRecords.UnitLookup lookup =
        funding
            ...
            .orElseThrow(
                () -> new IllegalStateException("Funded payment operation's unit is unavailable"));
  ...
  private void failUnit(UUID fundingUnitId, Instant now) {
    FundingRecords.UnitLookup lookup =
        funding
            ...
            .orElseThrow(
                () ->
                    new IllegalStateException("Declined payment operation's unit is unavailable"));
    FundingUnit unit = FundingUnit.rehydrate(lookup.unit());
  ...
  private static void requireUpdated(boolean updated) {
    if (!updated) {
      throw new IllegalStateException(
          "Payment relay result was applied to a concurrently modified row");
    }
  }
}
