...
class S3FulfillmentObjectStorage implements FulfillmentObjectStorage {
  ...
  public DirectDownload createDirectDownload(String objectKey, String objectVersion) {
    ...
    return new DirectDownload(
        URI.create(signed.url().toString()), clock.instant().plus(properties.downloadTtl()));
  }
  ...
  public VerifiedObject verify(String objectKey) {
    ...
    if (nativeChecksum != null && !nativeChecksum.isBlank()) {
      return new VerifiedObject(
          head.contentLength(), decodeChecksum(nativeChecksum), version, mediaType);
    }
    return hashObject(objectKey, version, mediaType, head.contentLength());
  }

  private VerifiedObject hashObject(
      String objectKey, String version, String mediaType, long expectedSize) {
    ...
    return new VerifiedObject(
        actualSize, HexFormat.of().formatHex(digest.digest()), version, mediaType);
  }
  ...
  private static String requireMediaType(String value) {
    if (value == null || value.isBlank()) {
      throw new IllegalStateException(
          "object storage must return a content type for media-type verification");
    }
    return value;
  }
