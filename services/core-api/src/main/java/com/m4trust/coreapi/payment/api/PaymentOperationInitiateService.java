...
class PaymentOperationInitiateService {
  ...
  PaymentOperationInitiateService(
  ...
  PaymentOperationView initiate(
    ...
    IdempotencyRequest idempotencyRequest =
        idempotencyRequest(context, fundingUnitId, expectedVersion, idempotencyKey);
    PaymentOperationView result =
        transactions.execute(
            status ->
                initiateInTransaction(
                    context, fundingUnitId, expectedVersion, idempotencyRequest, correlationId));
    if (result == null) {
  ...
  private PaymentOperationView initiateInTransaction(
      OperationContext context,
      UUID fundingUnitId,
      long expectedVersion,
      IdempotencyRequest idempotencyRequest,
      UUID correlationId) {
    FundingRecords.UnitLookup lookup =
        funding
            .findUnitByIdForUpdate(fundingUnitId)
    ...
    PaymentOperation operation =
        PaymentOperation.create(UUID.randomUUID(), fundingUnitId, providerKey, now);
    funding.insertOperation(operation.toRecord());
    funding.insertDispatch(
        new FundingRecords.DispatchRecord(
            UUID.randomUUID(),
            operation.id(),
            FundingRecords.DispatchType.INITIATE,
            providerKey,
            unit.amountMinor(),
            unit.currency(),
            now));
    audit.append(
        auditRecord(context, operation.id(), "PAYMENT_OPERATION_INITIATED", correlationId, now));
  ...
  private PaymentOperationView replay(IdempotencyResultReference reference, boolean buyerAdmin) {
    if (!IDEMPOTENCY_RESULT.equals(reference.type())) {
      throw new IllegalStateException("Unexpected idempotency result type");
    }
    FundingRecords.OperationLookup current =
        funding
            .findOperationById(reference.id())
            .orElseThrow(
                () -> new IllegalStateException("Replayed payment operation is unavailable"));
  ...
  private IdempotencyRequest idempotencyRequest(
      OperationContext context, UUID fundingUnitId, long expectedVersion, UUID key) {
    try {
      ...
      throw new IllegalStateException(
          "Cannot canonicalize payment operation initiate request", exception);
    }
  }

  private static void requireOperation(OperationContext context) {
    if (context.requestedOperation()
        != RequestedOperation.FUNDING_UNIT_PAYMENT_OPERATION_INITIATE) {
      throw new IllegalArgumentException(
          "Operation context does not match payment operation initiate");
    }
  }
}
