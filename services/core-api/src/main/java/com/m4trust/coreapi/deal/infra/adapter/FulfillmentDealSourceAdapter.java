...
import com.m4trust.coreapi.fulfillment.domain.FulfillmentSourcePorts;
...
class FulfillmentDealSourceAdapter implements FulfillmentSourcePorts.DealTarget {
  ...
  public Optional<FulfillmentSourcePorts.Target> findVisible(
  ...
  public Optional<FulfillmentSourcePorts.Target> lockVisibleForStart(
      OperationContext context, UUID dealId) {
    return deals
  ...
  private FulfillmentSourcePorts.Target target(OperationContext context, Deal deal) {
    ...
    return new FulfillmentSourcePorts.Target(
        deal.id(),
        ...
        fundingSummary.fundingStatus(),
        deal.currentRatificationPackageId(),
        ratified.evidencePolicy(),
        ratified.ruleReferences());
  }
  ...
  private RatifiedPackageProjection ratifiedPackage(OperationContext context, Deal deal) {
    ...
    return ratificationProjections
        .findCurrentPackage(context, deal.id(), deal.status().name(), currentPackageId)
        .filter(current -> "RATIFIED".equals(current.status()))
        .map(
            current -> {
              EvidencePolicy policy =
                  EvidencePolicy.effectiveFromSnapshot(
                      current.snapshot().schemaVersion(), current.snapshot().evidencePolicy());
              List<MilestoneRuleReference> refs =
                  current.snapshot().ruleSet().rules().stream()
                      .filter(
                          rule ->
                              "DELIVERY".equals(rule.category())
                                  || "QUALITY".equals(rule.category()))
                      .map(
                          rule -> new MilestoneRuleReference(rule.ruleReference(), rule.category()))
                      .toList();
  ...
  private record RatifiedPackageProjection(
      EvidencePolicy evidencePolicy, List<MilestoneRuleReference> ruleReferences) {
