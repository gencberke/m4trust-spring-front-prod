...
@Service
public class RatificationPackageCreateService {
  ...
  public RatificationPackageCreateService(
      ...
      RatificationSnapshotAssembler assembler,
  ...
  RatificationPackageReadDtos.Detail create(
    ...
    IdempotencyRequest idempotencyRequest =
        idempotencyRequest(context, dealId, request, idempotencyKey);
    return required(
  ...
  private RatificationPackageReadDtos.Detail createInTransaction(
    ...
    RatificationSourcePorts.Target target =
        deals.lockVisibleForCreate(context, dealId).orElseThrow(PackageNotFound::new);
    RatificationPackageRecord current =
        target.currentPackageId() == null
            ? null
            : packages
                .findByDealAndIdForUpdate(dealId, target.currentPackageId())
                .orElseThrow(
                    () -> new IllegalStateException("Deal current package is unavailable"));
    ...
    RatificationSourcePorts.Document document =
        documents.find(target.currentDocumentId()).orElseThrow(NotReady::new);
    RatificationSourcePorts.RuleSet ruleSet =
        ruleSets.find(target.currentRuleSetId()).orElseThrow(NotReady::new);
    ...
    if (current != null
        && current.status() == RatificationPackageStatus.PENDING
        && current.contentHash().equals(snapshot.contentHash())) {
      idempotency.recordResult(
          claim, new IdempotencyResultReference(IDEMPOTENCY_RESULT, current.id()));
    ...
    RatificationPackage created =
        RatificationPackage.create(
            UUID.randomUUID(),
            dealId,
            snapshotId,
            ...
            request.currency(),
            now);
    RatificationRepository.SnapshotRecord snapshotRecord =
        new RatificationRepository.SnapshotRecord(
            snapshotId,
            snapshot.snapshot().schemaVersion(),
            snapshot.serializedSnapshot(),
            snapshot.contentHash(),
            now);
    RatificationPackageRecord createdRecord = created.toRecord();
    ...
    idempotency.recordResult(
        claim, new IdempotencyResultReference(IDEMPOTENCY_RESULT, created.id()));
  ...
  private RatificationPackageReadDtos.Detail replay(
    ...
    RatificationPackageRecord record =
        packages.findByDealAndId(target.dealId(), reference.id()).orElseThrow(PackageNotFound::new);
    return reads.project(context, target, record);
  }

  private static RatificationPackageRecord withSnapshot(
      RatificationPackageRecord record, RatificationSnapshotAssembler.Result snapshot) {
  ...
  private static void validateCreateTarget(
      RatificationSourcePorts.Target target, CreateRatificationPackageRequest request) {
    ...
    if (target.currentDocumentId() == null
        || target.currentRuleSetId() == null
        || target.buyer() == null
        || target.seller() == null) throw new NotReady();
    if (request.amountMinor() < 1
        || request.amountMinor() > RatificationPackage.MAX_SAFE_INTEGER
        || request.currency() == null
        || !request.currency().matches("[A-Z]{3}")) throw new InvalidTerms();
  ...
  private AuditRecord audit(
      OperationContext context, UUID packageId, String action, UUID correlationId, Instant now) {
  ...
  private IdempotencyRequest idempotencyRequest(
      OperationContext context, UUID dealId, CreateRatificationPackageRequest request, UUID key) {
    return new IdempotencyRequest(
        context.authenticatedUserId(),
        context.tenantId(),
        IDEMPOTENCY_OPERATION,
        key,
        canonicalRequestHash(context, dealId, request));
  }

  private String canonicalRequestHash(
      OperationContext context, UUID dealId, CreateRatificationPackageRequest request) {
    try {
      ...
      throw new IllegalStateException(
          "Cannot canonicalize ratification package create request", exception);
    }
  }
  ...
  private static void requireOperation(OperationContext context) {
    if (context.requestedOperation() != RequestedOperation.DEAL_RATIFICATION_PACKAGE_CREATE) {
      throw new IllegalArgumentException(
          "Operation context does not match ratification package create");
    }
  }
