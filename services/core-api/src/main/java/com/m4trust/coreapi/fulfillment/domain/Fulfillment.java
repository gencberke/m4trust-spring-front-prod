...
public final class Fulfillment {
  ...
  public static Fulfillment rehydrate(FulfillmentRecord record) {
    return new Fulfillment(
        record.id(),
        record.dealId(),
        record.tenantId(),
  ...
  public void returnToEvidenceRequired(Instant changedAt) {
    ...
    if (status != FulfillmentStatus.REVIEW_REQUIRED) {
      throw new IllegalStateException(
          "fulfillment must be in REVIEW_REQUIRED to return to EVIDENCE_REQUIRED");
    }
    status = FulfillmentStatus.EVIDENCE_REQUIRED;
    updatedAt = Objects.requireNonNull(changedAt);
    version++;
  ...
  public FulfillmentRecord toRecord() {
  ...
  private void requireTransition(FulfillmentStatus next) {
    ...
    if (!allowed) {
      throw new IllegalStateException(
          "fulfillment status transition from " + status + " to " + next + " is not allowed");
    }
  }
