...
@ConditionalOnProperty(
    prefix = "app.payment.dispatch.relay",
    name = "enabled",
    havingValue = "true")
public class ReleaseDispatchRelay {
  ...
  ReleaseDispatchRelay(
      ReleaseDispatchStore store,
      SettlementRepository settlements,
      SettlementSourcePorts.CaseworkTarget casework,
      SettlementDealCompletionPort dealCompletion,
  ...
  public void relayOnce() {
    for (ReleaseDispatchStore.DispatchClaim claim :
        store.claimAvailable(properties.batchSize(), properties.claimTimeout())) {
      try {
        processClaim(claim);
      } catch (RuntimeException exception) {
        LOGGER.warn(
            "release dispatch deferred dispatchId={} operationId={}",
            claim.id(),
            claim.releaseOperationId(),
            exception);
      }
    }
  }
  ...
  private PaymentProviderPort.ReleaseProviderResult resolveOutsideTransaction(
    ...
    if (result.outcome() == PaymentProviderPort.ReleaseOutcome.NOT_FOUND) {
      SettlementRecords.ReleaseOperationLookup lookup =
          settlements
              .findOperationById(claim.releaseOperationId())
              .orElseThrow(
                  () -> new IllegalStateException("Dispatched release operation is unavailable"));
    ...
    transactions.executeWithoutResult(
        status -> {
          ...
          SettlementRecords.SettlementRecord settlementRecord =
              settlements.findByIdForUpdate(lookup.operation().settlementId()).orElseThrow();
          ...
          switch (result.outcome()) {
            case SIMULATED_SETTLED -> {
              ...
              requireUpdated(
                  settlements.updateOperation(operation.toRecord(), previousOperationVersion));
              long previousSettlementVersion = settlement.version();
              settlement.markSimulatedSettled(now);
              requireUpdated(
                  settlements.updateSettlement(settlement.toRecord(), previousSettlementVersion));
              audit(
                  tenantId,
                  operation.id(),
                  AUDIT_SUBJECT_RELEASE,
                  "RELEASE_OPERATION_SIMULATED_SETTLED",
                  now);
              audit(
                  tenantId,
                  settlement.id(),
                  AUDIT_SUBJECT_SETTLEMENT,
                  "SETTLEMENT_SIMULATED_SETTLED",
                  now);
              if (!activeDispute) {
                ...
                audit(
                    ...
                    dealId,
                    AUDIT_SUBJECT_DEAL,
                    "DEAL_COMPLETED_SIMULATED_SETTLEMENT",
                    now);
              }
            }
            case SIMULATED_DECLINED -> {
              ...
              audit(
                  ...
                  operation.id(),
                  AUDIT_SUBJECT_RELEASE,
                  "RELEASE_OPERATION_SIMULATED_DECLINED",
                  now);
            }
  ...
  private void audit(
      UUID tenantId, UUID subjectId, String subjectType, String action, Instant now) {
    audit.append(
        new AuditRecord(
            UUID.randomUUID(),
            tenantId,
  ...
  private static void requireUpdated(boolean updated) {
    if (!updated) {
      throw new IllegalStateException(
          "Release relay result was applied to a concurrently modified row");
    }
  }
}
