...
@Service
public class VideoAnalysisResultService implements VideoAnalysisTerminalEventPort {
  ...
  public VideoAnalysisResultService(
      ObjectMapper mapper,
      VideoAnalysisTerminalInboxPort inbox,
  ...
  private void applyInTransaction(JsonNode event, String eventType) {
    ...
    VideoAnalysisRepository.VideoAnalysisJobRecord job =
        repository.findByIdForUpdate(jobId).orElseThrow(VideoAnalysisTerminalViolation::new);
  ...
  private void complete(
      JsonNode event, VideoAnalysisRepository.VideoAnalysisJobRecord job, Instant now) {
