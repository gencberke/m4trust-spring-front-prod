...
import com.m4trust.coreapi.payment.domain.FundingSourcePorts;
...
class FundingDealSourceAdapter implements FundingSourcePorts.DealTarget {
  ...
  public Optional<FundingSourcePorts.Target> lockVisibleForCreate(
  ...
  private FundingSourcePorts.Target target(OperationContext context, Deal deal) {
    ...
    if (currentPackageId != null) {
      RatificationPackageProjectionPort.CurrentPackage current =
    ...
    return new FundingSourcePorts.Target(
        deal.id(),
