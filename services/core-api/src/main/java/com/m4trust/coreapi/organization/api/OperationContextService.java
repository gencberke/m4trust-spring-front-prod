...
/**
 * Single application-layer authority for legal-entity request context parsing and membership
 * verification.
 */
@Service
class OperationContextService {
  ...
  OperationContext resolve(
    ...
    UUID authenticatedUserId = parseAuthenticatedUserId(authenticatedUserIdValue);
    UUID activeLegalEntityId = parseActiveLegalEntityId(activeLegalEntityIdValue);
    if (pathMatchRequired) {
      UUID requestedLegalEntityId = parseRequestedLegalEntityId(requestedLegalEntityIdValue);
      if (!requestedLegalEntityId.equals(activeLegalEntityId)) {
        throw new LegalEntityAccessDeniedException();
      }
    }

    OrganizationRepository.ResolvedMembership membership =
        repository
            .findAuthorizedMembership(authenticatedUserId, activeLegalEntityId)
  ...
  private UUID parseAuthenticatedUserId(String value) {
    try {
      ...
      throw new IllegalStateException("Authenticated principal does not carry a UUID", exception);
    }
  }
