...
public final class AbsoluteSessionTimeoutFilter extends OncePerRequestFilter {
  ...
  public AbsoluteSessionTimeoutFilter(Clock clock, SessionSecurityProperties properties) {
    this.clock = clock;
  ...
  private boolean isAbsolutelyExpired(HttpSession session) {
    Instant deadline =
        Instant.ofEpochMilli(session.getCreationTime()).plus(properties.absoluteTimeout());
