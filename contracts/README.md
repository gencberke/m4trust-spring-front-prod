...
When an AI owner baseline is not supplied (`M4TRUST_AI_CONTRACTS_ROOT` / `M4TRUST_AI_CONSUMER_CHECK_CMD`), validation and CI emit exactly `UNVERIFIED_EXTERNAL_GATE: AI contract baseline not supplied` and continue; they never invent AI compatibility (ADR-019). Structural OpenAPI drift between documents can also be checked with `contracts/scripts/compare_openapi_structure.py` (including a deliberate negative fixture).
