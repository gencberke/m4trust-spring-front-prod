...
`openapi/core-internal-v1.yaml` is the private Core OpenAPI 3.1 surface for `GET /internal/v1/contracts`. It returns the ADR-016 projection (`service`, 40-hex `releaseRevision`, `contractBundleDigest`, ordinal `files[{path,sha256}]`) under HTTP bearer probe-token auth and carries no business data. The validator computes the deterministic contract-bundle digest over the ADR-016 inclusion globs (`asyncapi`/`openapi`/`schemas`/`examples` only; README/CHANGELOG/scripts/deps excluded) and prints `UNVERIFIED_EXTERNAL_GATE: AI contract baseline not supplied` unless `M4TRUST_AI_CONTRACTS_ROOT` points at a read-only AI baseline tree (per-file hash delta on mismatch; no eval path).
