...
`openapi/core-api-v1.yaml` is the reviewed OpenAPI 3.1 design contract for the same-origin public Spring API consumed by the frontend. It contains the reusable Problem Details foundation, Slice 1 authentication, Slice 2 legal entity and membership, Slice 3 Deal create/list/detail/update/cancel, Slice 4 invitation and cross-entity participation, Slice 5 atomic buyer/seller party-management, Slice 6 direct document-upload surfaces, and Slice 8 asynchronous current-document analysis surfaces, all designed before implementation. Slice 6 creates a JSON upload intent, transfers PDF/DOCX bytes directly between the browser and private object storage, and finalizes only independently verified storage metadata. It exposes retained PENDING_UPLOAD/AVAILABLE/SUPERSEDED history, opaque immutable object versions, and short-lived direct download links; it never exposes a Spring binary proxy or a permanent storage URL. Slice 8 adds an initiator-only, idempotent analysis request action and participant-readable status/result projection for the current AVAILABLE document. It keeps technical failures safe, exposes canonical extracted content as advisory review input only, and never treats AI output as business acceptance. The contract preserves centralized context resolution and non-disclosure, uses explicit conflicts and idempotent replay, and exposes backend-derived lifecycle and actor-aware action availability instead of asking the frontend to infer them. Party assignment remains DRAFT-only and participant-bound; no activation or ratification endpoint exists in this contract. Management endpoints outside these reviewed public operations are not part of the public contract.
