...
The envelope field `transactionId` identifies the owning `Deal` aggregate. The v1 name is retained for wire compatibility; renaming it to `dealId` is a v2 candidate, and Spring maps it at the integration boundary. Canonical `producer.service` values are the canonical service names (`m4trust-core-api`, `m4trust-ai-worker`).
...
The validator also computes the deterministic contract-bundle digest (`--print-digest` prints only that value). It checks every JSON Schema with Draft 2020-12 schema checks, resolves local canonical `$ref` references, checks unique IDs and exact event versions, validates every canonical JSON fixture, validates the AsyncAPI/OpenAPI YAML references and path invariants, requires the public error components, enforces the closed `ApiErrorCode` and `FieldErrorCode` catalogs via machine-readable `components.x-m4trust-api-error-ownership` exact-set matching (including OpenAPI↔Java comparison and rejection of open/`TOO_SHORT`/removed combined fulfillment codes), and runs expected-invalid checks for schema versions, non-UTC timestamps, error combinations, strict structured values, plus an expected-valid future optional field. It prints JSON paths for failures and exits non-zero on failure.

When an AI owner baseline is not supplied (`M4TRUST_AI_CONTRACTS_ROOT`), validation and CI emit exactly `UNVERIFIED_EXTERNAL_GATE: AI contract baseline not supplied` and continue; they never invent cross-repository AI compatibility and never `eval` owner shell commands. When the directory is supplied, the validator compares the contract-bundle inclusion set and fails with per-file expected/actual SHA-256 hashes on mismatch. Structural OpenAPI drift between documents can also be checked with `contracts/scripts/compare_openapi_structure.py` (including a deliberate negative matrix for path/parameter/security/status/media-type/schema-ref).

The same command runs in [GitHub Actions](../.github/workflows/contracts-validation.yml) for pushes to `main` and pull requests targeting `main` when contract, Core API main sources, generated frontend types, or workflow files change.
...
Slice 11 adds the provider-independent sandbox funding foundation. `POST /deals/{dealId}/funding-plan` is a buyer entity ADMIN-only, idempotent action whose request carries only the Deal `expectedVersion`; `amountMinor` and `currency` are never client-supplied and are always copied server-side from the RATIFIED package. It returns a synchronous `201 Created` with the `FundingPlanDetail` body (the plan plus its single `FundingUnit`) and a `Location` pointing back at the same singleton path; a Deal has at most one FundingPlan and a concurrent or repeated create races under the same database unique invariant, so a second attempt returns `409`. `GET /deals/{dealId}/funding-plan` is the participant-readable plan/unit/current-operation projection and returns `404` until a plan exists. `POST /funding-units/{fundingUnitId}/payment-operations` initiates a payment; `Idempotency-Key` is required, the request carries the FundingUnit `expectedVersion`, and the durable intent/dispatch/audit/idempotency record is committed in one short transaction before the provider is ever called. It returns `202 Accepted` with the `CREATED` `PaymentOperation` projection and a `Location` at `/api/v1/payment-operations/{paymentOperationId}`; the provider result is never awaited in-request. `GET /payment-operations/{paymentOperationId}` is the polling read. `POST /payment-operations/{paymentOperationId}/reconcile` accepts only an `UNCONFIRMED` operation and dispatches query-first reconciliation the same way: it commits a durable dispatch/audit/idempotency record and returns `202` with the same operation `Location`, never calling the provider in-request. A CREATED or UNCONFIRMED operation is a FundingUnit's only in-flight operation; a FUNDED unit or an operation with a different Idempotency-Key while one is in-flight both return `409`. `FundingStatus`, `FundingUnitStatus`, and `PaymentOperationStatus` are closed enums; UNCONFIRMED is explicitly never treated as failure, and `PARTIALLY_FUNDED` is part of the `FundingStatus` axis but unreachable in V1 because exactly one FundingUnit exists per Deal. `DealDetail.funding` (a null-tolerant `DealFundingSummary`) and the `canCreateFundingPlan`/`canInitiateFunding`/`canReconcilePaymentOperation` booleans on `DealAvailableActions` are additive optional members; absent or unknown action availability remains false/read-only. Existing Deal required fields and meanings are unchanged. `PaymentOperation` exposes only a safe non-raw projection (status, an explicit `reconciliationRequired` indication, and an opaque non-PII `providerReference`); raw provider payloads, card data, and credentials are never part of the public contract.

Slice 12 adds the fulfillment and evidence contract after a Deal is ACTIVE and FUNDED. `POST /deals/{dealId}/fulfillment` is a seller entity ADMIN or MEMBER idempotent action that atomically creates the Deal's single fulfillment record and single primary milestone, bound to the current RATIFIED package; a concurrent or repeated start races under the same database unique invariant and returns `409`. It returns synchronous `201 Created` with `FulfillmentDetail` and a `Location` at the singleton path. `GET /deals/{dealId}/fulfillment` is the participant-readable projection and returns `404` until fulfillment has started. Evidence upload reuses the Slice 6 intent/direct-PUT/finalize pattern but remains a separate aggregate and object namespace: `POST /deals/{dealId}/fulfillment/evidence/upload-intents` creates a `PENDING_UPLOAD` `EvidenceSubmission` and a short-lived presigned PUT target; `POST /deals/{dealId}/fulfillment/evidence/{evidenceSubmissionId}/finalize` verifies storage size, SHA-256, and immutable object version outside the DB transaction and atomically marks the submission `SUBMITTED`, sets it as the milestone's current evidence, and moves `FulfillmentStatus` to `REVIEW_REQUIRED`. `POST .../download-link` mints a short-lived pinned GET URL for any participant. Buyer entity ADMIN-only `.../accept` and `.../reject` actions carry both the Deal `expectedVersion` and the evidence submission `expectedEvidenceVersion`; they check the target under lock, preserve rejected evidence as immutable history, and accept completes only the milestone and fulfillment (`COMPLETED`) while the Deal remains `ACTIVE`. No payment release, settlement, payout, refund, provider call, or AI job is produced.

Slice 13 adds optional buyer entity ADMIN explicit video analysis for finalized
VIDEO MP4 or PHOTO JPEG/PNG evidence under the current advisory-analysis contract. `GET /deals/{dealId}/fulfillment/evidence/{evidenceSubmissionId}/video-analysis` is participant-readable and returns `NOT_REQUESTED` when no job exists for eligible evidence, or the latest job/result projection when one exists, with backend-derived `VideoAnalysisAvailableActions.canRequest`. `POST` on the same path is buyer entity ADMIN-only, requires CSRF and `Idempotency-Key`, accepts only `expectedEvidenceVersion`, returns `202 Accepted` with `VideoAnalysisDetail` and a `Location` pointing back at the GET resource, and never waits for RabbitMQ, storage download, or AI processing. Observations, anomalies, confidence, warnings, and `advisoryOutcome` are advisory only; they never approve or reject evidence, complete fulfillment, change the Deal, open a dispute, or release money. Manual buyer accept/reject remains independent and authoritative. The public result omits technical metadata, storage identifiers/URLs, event payloads, and provider/model details. PENDING or ineligible evidence type/media pairings remain a non-disclosing `404`.

Slice 14A adds the dispute and casework foundation after fulfillment has started. `POST /deals/{dealId}/disputes` is buyer or seller entity ADMIN-only, requires CSRF and `Idempotency-Key`, accepts only `reasonCode`, trimmed plaintext `subject` and `statement`, `expectedDealVersion`, and `expectedFulfillmentVersion`, and returns `201 Created` with `DisputeDetail` plus a `Location` at `/deals/{dealId}/disputes/{disputeId}`. Opening atomically captures the immutable `DisputeOpeningSnapshot` of fulfillment provenance, every finalized evidence record present at the lock point, and pinned successful advisory video results when they already exist; it never accepts client-supplied evidence, storage, AI-result, or settlement identity and never mutates Deal, fulfillment, evidence, funding, payment, settlement, provider, or AI state. Buyer/seller ADMIN and MEMBER users read active and withdrawn history through paginated list/detail and append comments while a case is `OPEN` or `UNDER_REVIEW`. Counterparty ADMIN `POST .../acknowledge` changes only `OPEN -> UNDER_REVIEW`; opening-entity ADMIN `POST .../withdraw` changes only `OPEN | UNDER_REVIEW -> WITHDRAWN` and retains case, snapshot, comments, and audit history. Initiator-only entities, other participants, and nonparticipants receive non-disclosing 404 from casework endpoints and no `DealDetail.casework` summary. Optional null-tolerant `DealDetail.casework` and `canOpenDispute` on `DealAvailableActions` are additive actor-aware members; absent or unknown action availability remains false/read-only.
...
`openapi/core-internal-v1.yaml` is the private Core OpenAPI 3.1 surface for `GET /internal/v1/contracts`. It returns the deployment contract-bundle projection (`service`, 40-hex `releaseRevision`, `contractBundleDigest`, ordinal `files[{path,sha256}]`) under HTTP bearer probe-token auth and carries no business data. The validator computes the deterministic contract-bundle digest over the committed inclusion globs (`asyncapi`/`openapi`/`schemas`/`examples` only; README/CHANGELOG/scripts/deps excluded) and prints `UNVERIFIED_EXTERNAL_GATE: AI contract baseline not supplied` unless `M4TRUST_AI_CONTRACTS_ROOT` points at a read-only AI baseline tree (per-file hash delta on mismatch; no eval path).
