...
→ 11B-B B-G0 real-provider probe/contract freeze
→ revised 11B-B human ready approval
→ 11B-B real staging funding
→ G1 evidence decision
→ 11B-B closure / C3-C4-G4c
...
→ one final planner-owned staging/provider E2E
→ 14B closure
```
...
| Work item | Plan phase | Owner/result |
| --- | --- | --- |
| Evidence gate | B-G0 | Planner/user authorized provider probes; no implementer packet from a planning plan |
| `11B-T01` | B-P1–B-P2 | Evidence-derived contract + adapter hardening |
| `11B-T02` | B-P3–B-P4 | Staging activation + funding recovery acceptance |
| none | B-P5 | Planner/human G1 evidence decision |
| `11B-T03` | B-P6 | Final real staging browser preparation/support; planner owns the run |
| none | B-P7 | Planner closure and gate synchronization |

B-P1 onward requires the post-B-G0 plan revision to be explicitly approved and
moved to `ready/`.
...
| --- | --- |
| R-P1 | Provider mappings, cardinality and lifetime identity fixed |
| R-P2 | Settlement/release/finality states and dispute races fixed |
...
| --- | --- | --- |
| `14B-T01` | I-P1 | Ratification v2 + immutable completion-time compatibility |
| `14B-T02` | I-P2 | Settlement persistence/cardinality/ports |
| `14B-T03` | I-P3 | Eligibility/read projection + explicit durable release intent |
| `14B-T04` | I-P4 | Provider dispatch + query-first recovery |
| `14B-T05` | I-P5 | Verified finality + Deal completion + frontend |
| `14B-T06` | I-P6–I-P7 | Focused hardening + final implementer handoff |
| none | P8 | Planner review, final E2E and closure |
...
7. Record external environment, provider identity and timestamps without
...
For external Railway/Moka evidence, preserve redacted deployment/probe IDs,
timestamps, call counts and outcome categories. Never copy credentials, raw
card/payment data, raw provider bodies or unnecessary PII into git.
...
| --- | --- | --- |
| G2/G3 | Document/ADR compatibility review only | None |
...
| 11B-B | Focused real-adapter/payment recovery checks | One final real staging funding browser flow |
| ADR-014 | Decision completeness and evidence traceability | None |
| 14B I-P1–I-P6 | Phase-specific contract/DB/eligibility/provider/finality/race checks | None |
| 14B I-P7/P8 | One final full verify/build, then planner review | One final staging/provider settlement browser flow |

Run a browser E2E only when at least one is true:

- a user-visible flow or authorization context materially changed;
- a new external process/provider boundary reaches the browser;
- same-origin cookie/CSRF/redirect behavior is the risk;
- the final money movement/finality flow cannot be accepted from focused tests;
...
- real-provider query/finality or duplicate behavior is inconclusive;
- an unknown operation would require a new provider identity;
- behavior needs optimistic settlement, approve-then-refund or invented consent;
- a schema-v1 client would be silently broken without accepted migration/versioning;
- `updated_at`, browser time, provider redirect or AI output is proposed as the
  contractual release clock/authority;
- implementation requires production credential, production money or an
  unresolved production legal/KYC/custody/fee/split decision;
