...
- `frontend`: generated OpenAPI types, TanStack Query-backed authentication and
  ...
  projections, filtering, sorting and pagination, stale-version recovery,
...
- `contracts/`: the reviewed Slice 4 public OpenAPI surface and existing
  Spring–AI contract foundations.

## Validation state

- Contract validation passes for the committed Slice 4 OpenAPI.
- Core API `mvn verify` passes against Testcontainers PostgreSQL.
- Frontend `npm run typecheck` and production `npm run build` pass.
- Slice 4 real two-browser acceptance passed on 2026-07-18.
...
Slice 5 Deal parties and activation is the next approved implementation plan
under `docs/plan/ready/`.
...
Implement Slice 5 Deal parties and activation.
